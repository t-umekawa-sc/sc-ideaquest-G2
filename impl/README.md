...
| SC-50/51/52 | 情報インプット（一覧/登録/詳細） | ⬜ | （Next.js 未移植） | **モック作成（2026-09-19・`doc/画面設計/mocks/SC-50_情報インプット.html`）**＝一覧(DataTable サーバー委譲前提＋ワードクラウド)＋SC-51 登録/編集(リッチエディタ・語プレビュー・続報プリフィル・属性付与折りたたみ)＋SC-52 詳細(要約・リッチ本文・属性・関連リンク種別・続報スレッド)。FR-41／設計 §12（続報/要約/WC同期/リッチテキスト）実体化済み。**Next.js 移植（`features/info-input`）→backend 新ドメイン `tenant/info`＋migration は未着手**。新規依存見込＝`nh3`(backend サニタイズ)／リッチエディタ(frontend) |
