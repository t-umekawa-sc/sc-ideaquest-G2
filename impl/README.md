...
| SC-50/51/52 | 情報インプット（一覧/登録/詳細） | 🟨 | **一覧＝backend 接続済（Phase A）／詳細＝backend 接続済（Phase B・読み取り）／登録・編集＝fixtures（Phase C 未着手）** | **モック→Next.js 移植→backend Phase A 一覧結線（2026-09-20）→Phase B 詳細結線（2026-09-21）**。**一覧＝サーバー委譲（`GET /info-items`・§1.8.1・`app/tenant/info` 4層＋migration 0028）**＝状態タブ(facets)／続報束ね(`roots_only`)／ワードクラウド(`GET /info-items/word-cloud`)／全文検索(server `q` PGroonga)。**詳細＝`GET /info-items/{id}`（SC-52・InfoDetailView 結線）**＝全属性＋関連リンク〔`target_title` を ideas/quests から解決〕＋続報スレッド＋ミニ・ワードクラウド＋`can`（`edit_content`＝作成者／`curate`＝curator／`add_link`＝全員・サーバー算出）。列 flags は backend ホワイトリスト一致。demo seed＝bootstrap `seed_demo_info`（i1〜i5＋filler30・リンクは実 idea/quest を指す・冪等）。／**登録・編集・続報（`POST`/`PATCH`・nh3・janome・要約）＝Phase C**（詳細の編集導線は `can` で出し分け・現状は無効化）。編集/属性制御の仕様は確定済（API N.0/SC-50/データモデル）。新規依存見込＝`nh3`／リッチエディタ本実装。 |
