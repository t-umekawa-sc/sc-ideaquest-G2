...
> 進捗の最終確認: **2026-09-26（変更履歴の標準装備＝全エンティティ完了）**。全エンティティ（アイデア/情報/コンセプト/振り返り/クエスト/評価）に**変更履歴を標準装備**（設計ドラフト [変更履歴標準](../doc/設計ドラフト/変更履歴標準.md)・ISO 56001 の反復更新で当時の判断材料/判断結果を追う）。**2系統**＝①内容の版（per-entity `*_revisions`・JSONB スナップ・無変更は版なし）②意思決定ログ（`*_decision_log`・status/decision 遷移）。**共通機構**＝backend `app/tenant/_shared/revisions.py`（FieldSpec 駆動の差分エンジン）／frontend `components/ui/RevisionTimeline.tsx`（variant idea=リンク/info=折り畳み）。migration **0037**（コンセプト＝内容版＋意思決定ログ＋判断材料スナップ）・**0038**（振り返り）・**0039**（クエスト定義＋ステータスログ・履歴FK CASCADE）・**0040**（評価＝確定起点・ア/コ両評価）。UI＝各詳細に「🕘 更新履歴」（コンセプト/クエスト=モーダル・リンク／振り返り/評価=折り畳み）。**backend 全ドメイン 812 passed・TC ✅（836件）・frontend build OK**。ブランチ `feature/chat-thread-independence`・**PR #2**（受入待ち）。以下の同日前半＝チャット独立化のスナップショット。
>
> 進捗の最終確認: **2026-09-26（チャット）**（**チャット中核を `chat_thread` 親テーブルで完全独立化〔案X・migration 0035〕＋コンセプト議論チャットをアイデアと完全同一（フル機能パリティ）に**）。チャット中核（messages/reads/reactions＋mentions/quotes/attachments/pins）は **`thread_id` ただ一つ**で動くホスト非依存サブシステムへ一般化（旧 `chat_group_id`/`concept_chat_scope_id` の2列＋CHECK を撤去）。`chat_thread(owner_type, owner_id)`＝ポリモーフィック所有（`idea`→chat_groups.id / `concept_scope`→concept_chat_scopes.id）。**新ホスト追加は thread を1本持つだけ＝中核無改修**（正本 [データモデル §5.14b](../doc/データモデル.md)）。コンセプト側は共有 `IdeaChatView` を **source 抽象**（`features/chat/source.ts`＝`ideaSource`/`conceptScopeSource`）で駆動＝reactions/魔法/メンション/引用/ピン/添付/未読/リアルタイムまで同一（slice A 破棄）。**backend 全ドメイン緑（790+ passed）・TC ✅（816件・新 P-TC-510/511/512）・frontend build OK**。ブランチ `feature/chat-thread-independence`・**PR #2**（受入待ち）。以下の 2026-09-24 以前は当時のスナップショット。
>
> 進捗の最終確認: **2026-09-24**（**FR-41 情報インプット＝成果物連携〔SC-12/SC-22 関連情報ストリップ＋逆向き追加〕＋Phase2 採否〔`disposition`＝採用/不採用/未処理・pending 以外ロック・SC-52 採否モード・クエスト結果タブに採用情報集約〕まで完了**・migration 0031/0032・TC トレーサビリティ 755件。詳細は本節末「情報インプット」）。以下の 2026-08-27 記述は当時のスナップショット。
>
...
| SC-22 | アイデア詳細 | ✅ | `(app)/ideas/[ideaId]` | 本体＋投票/フォロー（D.5/D.6）＋添付（D.3）＋quest 参照（D.1）＋更新履歴（D.4）＋評価結果/選定（F.1/F.3）＋**チャット活発度/プレビュー（E.1・`getChatActivity`/`getChat`）**。**FR-41＝概要直下に関連情報ストリップ（D related-info・SC-12 と同 strip・採否/逆向き追加/SC-52 採否モード）**。全セクション実接続 |
| SC-24 | アイデアチャット | ✅ | `(app)/ideas/[ideaId]/chat` | **実接続**（`getChat`/`postMessage`/`editMessage`/`deleteMessage`/`markRead`/`addReaction`/`removeReaction`/`getSpells`）＝投稿/編集/削除・リアクション/魔法・メンション（実メンバー）・添付（署名DL）・**複数引用**（`chat_message_quotes`・§5.16b）・既読・comment 権限/completed 凍結。**リアルタイム(L) 接続済み＝chat:{thread_id} 購読で新着/編集/削除/リアクションを即時反映（WS 受信で再取得）**。**チャット独立化（§5.14b・2026-09-26）＝共有 `IdeaChatView` を source 抽象（`ideaSource`）で駆動。コンセプト議論（SC-61）も同一コンポーネント＝完全同一機能** |
| SC-25 | 評価画面 | ✅ | `(app)/ideas/[ideaId]/eval`＋`@modal/(.)ideas/[ideaId]/eval` | 5観点採点＋観点別コメント＋総評＋公開範囲＋集計プレビュー（F.2）。`getMyEvaluation` プリフィル・確定/下書き＝`putEvaluation`・422/403/409 はサーバー権威。**URL 付きモーダル化済み**＝SC-22 ソフト遷移＝Intercept モーダル（`EvaluationModal`）／URL直・リロード＝フルページ（`EvaluationView` が `onClose` で chrome 出し分け）。**変更履歴（2026-09-26）＝「🕘 確定履歴」折り畳み＝確定(submit)ごとの版（0040・アイデア/コンセプト両評価）** |
...
| SC-50/51/52 | 情報インプット（一覧/登録/詳細） | 🟩 | **Phase A〜D＋成果物連携＋Phase2 採否 完了（backend 接続済）＝FR-41 コア完成**。一覧/詳細/登録・編集・リンク・画像・参考資料/アーカイブ・続報・この情報からクエスト作成・raw物理削除・curator 付与・**反証→揺さぶり通知**（要再評価フラグ/リセット＝コンセプト段の今後）／**成果物連携＝SC-12/SC-22 の関連情報ストリップ表示＋逆向き追加（既存 EP 流用）**／**Phase2 採否＝`disposition`（採用/不採用/未処理）を管理権限者が設定〔`PATCH /{quests,ideas}/{id}/related-info/{link_id}`〕・pending 以外はロック（棄却/種別変更 409）・SC-52 は `?from=種別:ID` で採否モード（参照＋扱い入力）・情報側リンク編集は採否済みを🔒＋コンボ非活性・migration 0032** | **モック→Next.js 移植→Phase A 一覧（2026-09-20）→Phase B 詳細（2026-09-21）→Phase C 変更系（2026-09-21）**。**一覧＝サーバー委譲（`GET /info-items`・§1.8.1・`app/tenant/info` 4層＋migration 0028）**＝状態タブ(facets)／続報束ね(`roots_only`)／ワードクラウド／全文検索(PGroonga)。**詳細＝`GET /info-items/{id}`**＝全属性＋関連リンク〔title 解決〕＋続報スレッド＋ミニ・ワードクラウド＋**参考資料**＋`can`（`edit_content`＝作成者／`curate`＝curator／`add_link`＝全員）。**Phase C＝登録/続報（`POST`・nh3/janome/要約・親リンク複製）／内容編集（`PATCH`＝作成者・再派生＋版履歴 migration 0029）／キュレーション（`PATCH`＝curator・raw→curated）／関連リンク（候補検索＋追加/種別変更/棄却・全員）／貼付画像の MinIO 再ホスト（`POST /info-items/images`・paste ハンドラ・§12-4）／参考資料（`POST`/`DELETE .../attachments`・作成者・migration 0030・§5.33）**。詳細はインライン編集を `can` で出し分け。**Phase D＝アーカイブ/解除（`POST .../archive`・`/unarchive`＝curator・論理削除〔監査保持〕・状態タブに「アーカイブ」追加・facets.archived・詳細本文 curator ブロック/行メニューから操作）／続報登録（親を実 API でプレビュー・`POST /info-items {parent_info_id}`＝登録時に親の未棄却リンクを origin=auto で自動複製・スレッドは根に紐づけ・詳細/行メニューから起票）／この情報からクエスト作成（`POST /quests {from_info_id}`＝作成後にサーバーが info_link〔quests・related・manual〕を自動生成＝逆リンク・下書き作成→SC-11 で仕上げ・C.2）／未判定の物理削除（`DELETE /info-items/{id}`＝登録者本人・raw のみ・従属行＋MinIO 除去・curated は 409〔archive 誘導〕・続報ありは 409・行 ⋯ メニューから）／情報判定権限の付与・剥奪（`GET/POST/DELETE /info-curators`＝会社アカウント管理者/system_admin・account_id 識別・SC-93 `/admin/accounts` の `InfoCuratorSection` に同居・N.5）／**反証（refuting）→揺さぶり通知**（`POST` 起票 or `PATCH` で related/supporting→refuting 遷移時に post-commit dispatch＝`info_refuting_raised`／宛先=作成者/所有者＋評価者〔投票者〕＋クエスト管理者・本人除外／通知は既存 H 基盤・frontend は generic 表示＝icon⚠️・§N.6・通知のみ MVP）**。**編集導線は詳細（SC-52）のインライン編集に一本化**＝`/info-items/{id}/edit` ルート撤去・`InfoFormModal`/`InfoFormPanel` は新規/続報専用・行 ⋯「内容・属性を編集」→詳細へ・**api.ts の fixtures 全撤去＝FR-41 は fixtures 残渣ゼロ**。demo seed＝bootstrap `seed_demo_info`（i1〜i5＋filler30・冪等）。テスト＝pytest `tests/info` 49 green＋front unit（api.test.ts 7）。 |
...
| チャット（E） | `tenant/chat` | ✅ **8 EP**（`GET chat`〔一覧＋未読〕・`GET chat-activity`・`POST/PATCH/DELETE chat-messages`・`POST chat/read`＋**`POST/DELETE chat-messages/{id}/reactions`**〔通常/魔法・E.4〕）＝投稿/編集/削除・既読・活発度・添付・メンション・投稿XP+5・リアクション（マスタ絵文字）・魔法（1メッセージ1魔法/1チャット1回）。公開で chat_group 自動作成。migration 0013。**通知(H)結線済み（mention/idea_comment/follow_comment/magic_reaction）／リアルタイム(L)結線済み＝post-commit で `chat.*` を `chat:{thread_id}` へ publish（created/updated/deleted/reaction added/removed）**。**チャット中核を `chat_thread` で完全独立化（案X・migration 0035・2026-09-26）＝messages/reads/reactions は `thread_id` ただ一つで所属（旧2列＋CHECK 撤去）・門番は `_resolve_host` で owner_type 分岐に集約。コンセプト議論の rich EP を追加＝`GET/POST /concept-chat-scopes/{id}/chat`・`chat-activity`・`chat-messages`・`chat/read`（reaction/edit/delete/pin は message-id 共通 EP がそのまま効く＝ホスト非依存）。P-TC-510/511/512** |
...
| リアルタイム（L） | `tenant/realtime` | ✅ **WS `GET /api/v1/realtime`**（Cookie セッション認証＋Origin 検証）＝プロセス毎ハブ（`redis.asyncio` PSUBSCRIBE `notifications:*`/`chat:*`＋`realtime:revoke`・購読テーブル topic→接続・`company_id` フィルタで cross-tenant 遮断）。`notifications:{user_id}` 自動購読／`chat:{thread_id}` は動的購読（門番＝REST と同一・gate.py＝owner_type で idea/concept 分岐・§5.14b）。**発行＝H（notify post-commit）・E（chat post-commit）・C（除去で `publish_revoke`＝L.4 購読ドロップ）**。配信専用（書き込みは REST）。lifespan でハブ起動/停止。フロント＝`lib/realtime.ts`（単一 WS・再接続）＋`RealtimeProvider`（ベル）／SC-02・SC-24 は WS で再取得 |
...
**情報インプット機能＝設計完了・frontend 移植済／backend Phase A（一覧）結線済（2026-09-20・FR-41）**＝外部WEB情報を手動貼付→属性→アイデア/コンセプト/クエストへ動的リンク（差別化の核）。設計4点は実体化済み＝要件[FR-41](../doc/要件定義/README.md#6-機能要件)／データモデル[§5.33-5.37](../doc/データモデル.md)（`info_items`/`info_item_categories`/`info_links`/`info_tokens`/`info_curators`＋`info_*` enum）／API [ドメイン N](../doc/API設計/N_情報インプット.md)／テスト[N_情報インプット](../doc/テスト/N_情報インプット.md)（N-TC-001〜009・101〜107・pytest 16 green）／画面[SC-50/51/52](../doc/画面設計/screens/SC-50_情報インプット.md)。**進捗＝Phase A〜D＋成果物連携＋Phase2 採否まで完了（2026-09-24）**。一覧（サーバー委譲）／詳細／登録・編集・続報／リンク・画像・参考資料／アーカイブ／raw削除／curator付与／反証通知に加え、**成果物連携＝SC-12/SC-22 の関連情報ストリップ（read=C.8b／D・逆向き追加＝既存EP流用）**、**Phase2 採否＝`info_links.disposition`（採用/不採用/未処理・migration 0032）を成果物側の管理権限者が設定〔`PATCH /{quests,ideas}/{id}/related-info/{link_id}`〕・pending 以外ロック（棄却/種別変更 409）・SC-52 は `?from=種別:ID` で採否モード（参照＋扱い入力・機会特定/アーカイブ/続報スレッド非表示）・情報側リンク編集は採否済みを🔒＋コンボ非活性・クエスト結果タブに採用情報を集約（C.8 adopted_info＝クエスト＋配下アイデア）**。テスト＝pytest `tests/info` 68＋採否 C-TC-289〜295・D-TC-239/240・N-TC-225／設計判断はメモリ `fr41-disposition-phase2`。**残＝Phase2 の割り振り＋議論チャットはコンセプト機能のチャット一般化と統合（単独実装しない）**。`info_link_target=concepts/assumptions` はコンセプト段で実体化。
