...
> - **E群（チャット）**: 🟡 seed 済み・**受入中**（`seed_demo.py e`）＝Eチャットデモに 引用/メンション/リアクション/魔法/添付/未読 を投入。**受入指摘 DFT-E-006〜011＋ホバー操作メニュー改善（E-TC-214〜222/218）＝✅受入OK（2026-09-16）**（引用ジャンプ着地/入力欄 z-index/最小化引用展開/自分投稿の未読抑止/全件未読初期スクロール/可視ベース既読）。**残＝SC-24 基本（E-TC-201/203）・completed 凍結の受入**
> - **SC-01 ダッシュボード（UI要望）＝✅受入OK（2026-09-16）**: 「最近の通知」に既読ボタン・下段1:1・並び替え・通知フォント衝突解消（未スコープ `.notif-title` と衝突→`notif-subject` へ改名）（**I-TC-144 green**・コミット `5c5b539`）
