...
> - **E群（チャット）**: 🟡 seed 済み・**受入中**（`seed_demo.py e`）＝Eチャットデモに 引用/メンション/リアクション/魔法/添付/未読 を投入。**受入指摘 DFT-E-006〜011 を修正済み**（引用ジャンプ着地/入力欄 z-index/最小化引用展開/自分投稿の未読抑止/全件未読初期スクロール/可視ベース既読・回帰テスト **E-TC-214〜222 green**）＋ホバー操作メニュー改善（E-TC-218）。**ユーザー再確認待ち**（コミット `5c5b539`）
> - **SC-01 ダッシュボード（UI要望）**: 🟡 「最近の通知」に既読ボタン・下段1:1・並び替え・通知フォント衝突解消（未スコープ `.notif-title` と衝突→`notif-subject` へ改名）を対応済み（**I-TC-144 green**・コミット `5c5b539`）。**ユーザー再確認待ち**
> - **G群（魔法/ショップ/アバター/ランキング/実績）**: 🟡 seed 済み・**受入待ち**（`seed_demo.py g`）＝owner に 魔法解放済/アイテム購入・装備/実績獲得 を投入（3D=K-TC-015 ✅）。※ゲーム層UIは owner の game_mode を ON にして確認
> - **F群（評価）**: 未着手
...
- [ ] **SC-24 チャット E（E-TC-201/203）**＝クエスト「【受入】D-アイデア」内「Eチャットデモ」のチャット（`{APP}/ideas/<Eチャットデモ id>/chat`・seed 出力の URL が正）で: メッセージ投稿/編集/削除・**複数引用（owner の返信が u2/u3 を2件引用）**・**@メンション**（u3→owner・owner→u2/u3）・**👍通常リアクション**（u3）・**魔法リアクション**（owner＝SP保有者が付与済み。u2/u3 はSP0のため owner が担当）・**📎添付DL**（u2 の「議事メモ.png」）・**既読セパレータ**（初回オープンで未読区切り）・comment 権限。※自分でも SC-32 で解放した魔法をチャットで使える。
- [ ] **SC-24 チャット completed 凍結**＝完了クエスト「請求書処理の電子化デモ」のチャットで入力欄が**凍結バナー**（投稿/編集/削除/リアクション不可・過去ログ閲覧のみ）。
- [ ] **SC-24 受入指摘 DFT-E-006〜011（E-TC-214〜222・コミット `5c5b539`）**＝ブラウザ受入で認定した不具合を回帰テスト同梱で修正（全 green・**ユーザー再確認待ち**）:
