...
| SC-50/51/52 | 情報インプット（一覧/登録/詳細） | 🟨 | **一覧＝backend 接続済（サーバー委譲・Phase A）／詳細・登録・編集＝fixtures（Phase B/C 未着手）** | **モック（2026-09-19）→ Next.js 移植（2026-09-19）→ backend Phase A 一覧結線（2026-09-20）**。**一覧＝サーバー委譲（`GET /info-items`・DataTable §1.8.1 番号ページャ・`app/tenant/info` 4層＋migration 0028）**＝状態タブ(server status＋件数 facets)／続報束ね(`roots_only`)／ワードクラウド(`GET /info-items/word-cloud`)／全文検索タブ(server `q`＝title＋body_text PGroonga)。列 flags は backend ホワイトリスト一致（sort=created_at/title/status/priority/due_date/link_count・enum filter=priority/source/impact_class）。demo seed＝bootstrap `seed_demo_info`（i1〜i5・続報・機会/脅威・リンク・トークン・冪等）。／SC-52 詳細・SC-51 登録/編集/続報・この情報からクエスト作成＝**当面 fixtures**（URL 付きモーダル・型 OpenAPI 準拠）。**残＝Phase B `GET /info-items/{id}` 詳細結線／Phase C `POST`/`PATCH`（nh3・janome・要約）＋フォーム結線／編集/属性制御の仕様整理（Phase C 前ゲート）**。新規依存見込＝`nh3`(backend サニタイズ)／リッチエディタ本実装(frontend) |
