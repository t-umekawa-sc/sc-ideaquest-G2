...
def get_ideas(account_id, company_id, quest_id, *, status=None, limit, cursor=None) -> dict:
    ...
    with get_tenant_session(company.db_identifier) as ts:
        ...
        my_votes = repo.get_votes_for_ideas(ts, [r.id for r in rows], user.id)  # viewer の投票を一括（N+1 回避）
...
def _build_detail(ts, idea, viewer_id) -> dict:
    ...
    quest_ref = {
        ...
        "purpose": quest.purpose if quest else None,  # クエストの目的・テーマ（SC-25 評価文脈・社内レビュー）
    }
    return {
        "id": str(idea.id),
        "title": idea.title,
