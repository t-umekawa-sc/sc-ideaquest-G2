...
export function useAccountsChangedReload(eventName: string, reload: () => void | Promise<unknown>): void {
  useEffect(() => {
    const timers: ReturnType<typeof setTimeout>[] = [];
    const onChanged = () => {
      void reload();
      for (const t of timers.splice(0)) clearTimeout(t);
      for (const ms of FOLLOWUP_DELAYS_MS) timers.push(setTimeout(() => void reload(), ms));
    };
