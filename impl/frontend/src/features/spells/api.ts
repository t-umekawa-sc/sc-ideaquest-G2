...
export function unlockSpell(spellId: string): Promise<{ spell_id: string; unlocked: boolean; skill_point_balance: number } | null> {
  // 冪等キー付き（§1.9）＝二重送信でも SP は一度しか消費されない（サーバーが最初の結果を再生）。
  return apiFetch<{ spell_id: string; unlocked: boolean; skill_point_balance: number }>(`/spells/${spellId}/unlock`, { method: "POST", headers: idempotencyHeader() });
}
