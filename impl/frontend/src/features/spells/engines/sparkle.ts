// キラキラエンジン（受入済みモック style-guide.html §17L-h＝ライト版の TS 移植・GF-AC-091）。
// 明色パネル向け＝昼の光のきらめき。流れ星（金）→中央着弾でキラキラが四方へ弾け→彩色（金/水色/桃/紫/翠）の
// きらめきがまばらに瞬き続ける（＋たまにドット絵UFO・UFO の軌跡から星が推進のように噴き出す）。
// 夜空前提（天の川・オーロラ・白いきらめき）のダーク版（§17h）は明色パネルで見えないため置換。
...
// 金/水色/桃/紫/翠＝白パネルで映える濃いめの彩色。
const COLS = ["245,182,40", "60,170,235", "235,95,170", "150,95,230", "40,190,175"];
const STAR_COL = "240,175,45"; // 流れ星＝金
...
export function createSparkleEngine(opts: EngineOpts): SpellEngine {
  ...
  // 彩色のきらめき＝色付きグロー＋色付き4方向＋白い芯（白パネルで映える）。
  function drawGlint(x: number, y: number, size: number, alpha: number, col: string) {
    ...
    g.addColorStop(0, `rgba(${col},${(alpha * 0.95).toFixed(3)})`);
    g.addColorStop(0.5, `rgba(${col},${(alpha * 0.30).toFixed(3)})`);
    g.addColorStop(1, `rgba(${col},0)`);
    ctx.fillStyle = g; ctx.beginPath(); ctx.arc(x, y, size * 1.5, 0, 6.2832); ctx.fill();
    ctx.strokeStyle = `rgba(${col},${alpha.toFixed(3)})`; ctx.lineCap = "round";
    ...
    ctx.lineWidth = Math.max(0.6, size * 0.1); ctx.strokeStyle = `rgba(${col},${(alpha * 0.5).toFixed(3)})`;
    const d = size * 0.5;
    ctx.beginPath(); ctx.moveTo(x - d, y - d); ctx.lineTo(x + d, y + d); ctx.moveTo(x - d, y + d); ctx.lineTo(x + d, y - d); ctx.stroke();
    ctx.fillStyle = `rgba(255,255,255,${(alpha * 0.85).toFixed(3)})`; ctx.beginPath(); ctx.arc(x, y, Math.max(0.8, size * 0.26), 0, 6.2832); ctx.fill();
  }
  function drawStar() {
    if (!star) return;
    const tr = star.trail;
    for (let i = 0; i < tr.length - 1; i++) {
      ...
      ctx.globalAlpha = al; ctx.strokeStyle = `rgba(${STAR_COL},1)`; ctx.lineCap = "round"; ctx.lineWidth = 0.8 + (i / tr.length) * 3.2;
      ctx.beginPath(); ctx.moveTo(a[0], a[1]); ctx.lineTo(b[0], b[1]); ctx.stroke();
    }
    ctx.globalAlpha = 1;
    drawGlint(star.x, star.y, 7, 1, STAR_COL);
  }
  function spawnUFO() {
  ...
  function updateUFO() {
    if (ufo) {
      ...
      computeUFO();
      // 通った軌跡から星が推進（排気）のように噴き出す。
      if (u.prog > 0.02 && u.prog < 0.98) {
        const np = 1 + (rng() < 0.6 ? 1 : 0);
        for (let s = 0; s < np; s++) spawnTrailSparkle(u.x - u.dir * (3 + rnd(6)), u.y + (rnd(6) - 3), u.dir);
      }
  ...
  function drawUFO() {
    ...
    const bg = ctx.createRadialGradient(x, y + 9, 0, x, y + 9, 20); // 下向きの淡い光
    bg.addColorStop(0, `rgba(90,170,235,${(0.22 * bgFade).toFixed(2)})`);
    bg.addColorStop(1, "rgba(90,170,235,0)");
    ...
    for (const cell of UFO_CELLS) {
      const col = cell.char === "L" ? (Math.sin(t * 0.4 + cell.x) > 0 ? "#ffc233" : "#2f7fd8") : cell.color!;
  ...
  function spawnBurst(x: number, y: number) {
    for (let i = 0; i < 30; i++) {
      const ang = rnd(6.283), sp = 1.4 + rnd(4.2);
      burst.push({ x, y, vx: Math.cos(ang) * sp, vy: Math.sin(ang) * sp, life: 1, size: 2 + rnd(3.4), col: pick() });
    }
  }
  // UFO の推進＝軌跡から星が後方＋上下外側へ勢いよく噴き出す（排気のような噴射）。
  function spawnTrailSparkle(x: number, y: number, dir: number) {
    const side = rng() < 0.5 ? 1 : -1;
    const back = -dir * (0.5 + rnd(1.2));
    const out = side * (0.9 + rnd(2.2));
    burst.push({ x, y, vx: back + (rnd(1) - 0.5) * 0.5, vy: out, life: 1, size: 1.5 + rnd(2.3), col: pick() });
  }
  ...
  function spawnTwinkle() {
    // 1回の spawn で 2 粒＝密度アップ。
    for (let n = 0; n < 2; n++) twinkles.push({ x: rnd(W), y: rnd(H), vx: (rnd(2) - 1) * 0.1, vy: (rnd(2) - 1) * 0.08 - 0.03, age: 0, dur: 24 + (rnd(40) | 0), size: 1.8 + rnd(3.2), col: pick() });
  }
  ...
  function render() {
    ...
    drawUFO();
    for (let i = 0; i < twinkles.length; i++) { const w = twinkles[i], a = Math.sin((w.age / w.dur) * 3.14159); drawGlint(w.x, w.y, w.size, a, w.col); }
  ...
  function frame(ts: number) {
    ...
    if (state === "incoming" && star) {
      ...
      if (dist <= SPEED) { spawnBurst(CX, CY); star = null; state = "persist"; spawnUFO(); } // 着弾直後に一度UFOを走らせる
      else {
