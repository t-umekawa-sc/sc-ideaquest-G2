...
| E-TC-228 | api | 編集時のメンション通知整合＝**追加分のみ通知**（[C]②・決定A・2026-09-17） | パーティー員 U1/U2・U1 をメンションして投稿（U1 に mention 1件） | `PATCH`（mentions=[U1,U2]）→ さらに `PATCH`（mentions=[U2]） | 追加 U2 に `mention` 1件／不変 U1 は再通知なし（1件のまま）／除外しても取消しない（U1 は 1件のまま・U2 は不変で 1件のまま） | E.2／E.6（編集整合・決定A） |
