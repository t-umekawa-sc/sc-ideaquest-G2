...
| G-TC-154 | unit(front) | オーラの活力の粒（中央発の放射） | なし | `auraMotes()` | 12粒／中央から四方八方（dx に +と−・dy に +と−）／半径は3段(74/94/114相当)でばらつく／決定的 | GF-AC-091／#10 |
| G-TC-154 | unit(front) | 氷のヒビ伝播ツリー（実寸で生成） | 枠の実寸 w×h | `iceCrackTree(w, h)` | 四隅から t1×4→枝分かれ t2×8→t3×8→末端 t4×4／致命ヒビ(fa/fb/fc)は既存節点を左→右になぞる折れ線5本／各セグメントの left/top(%) は有限値／決定的（同 w,h で同結果） | GF-AC-091／#10 |
