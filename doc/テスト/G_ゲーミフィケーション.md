...
| G-TC-318 | e2e | SC-30 一覧はサーバーモード委譲＋状態の URL 復元（§1.8.1・§4.5⑨） | ログイン | `/shop`→リスト表示→レアリティ ヘッダでソート→きせかえへ→戻る | 初回/操作とも `GET /items?...per_page=&sort=` をサーバーへ委譲（リクエストで確認）・`sc30-shop.sort=` が URL に載り「並び替えを解除」チップ表示・詳細往復（pop）でソート状態が復元 | G.1／§1.8.1／§4.5⑨ |
