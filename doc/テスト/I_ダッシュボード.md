...
| I-TC-157 | e2e | 「自分のクエスト」を参加中と分離＋下書きカードは編集ダイアログ導線（ユーザー要望） | ACME owner（自作クエストあり）でダッシュボード | 表示／`GET /dashboard`／下書きカードの href | `section[自分のクエスト]` が出る・`quests[].is_owner` が真偽で自作/参加中が混在（C-TC-247）・下書きカード href が `?edit=1`（アイデア）/`/edit`（クエスト）/`/eval`（評価）＝詳細直リンク廃止 | SC-01／C.1 |
...
| I-TC-154 | 時間帯→挨拶の決定（#31 時間帯の挨拶） | unit | `greetingFor(hour)`（純関数） | 各時間帯の境界（4/5/10/11/17/18/23/0）・負/NaN/24以上 を与える | 5〜10=おはようございます／11〜17=こんにちは／18〜4=こんばんは。負/24以上は 24 で正規化（例 25→1→こんばんは・-1→23→こんばんは）／NaN は正午扱い（こんにちは） | SC-01 時間帯の挨拶（ゲーム感 #31・E 時間/環境） |
| I-TC-156 | 通知の相対時刻ラベル/グループ（SC-01 日時表示・SC-02 と共有） | unit | `timeLabel(iso,now)`／`groupOf(iso,now)`（now 固定で決定的） | 30秒前/5分前/3時間前(同日)/24時間前/3日前 | たった今／5分前／3時間前／昨日 hh:mm／M/D。groupOf＝today/yesterday/earlier を境界判定 | SC-01 通知日時表示・SC-02 と共有（`features/notifications/time.ts` へ集約） |