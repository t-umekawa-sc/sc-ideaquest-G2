...
| B-TC-176 | api | ゲームモード会社既定 `game_mode_default` の更新＋未上書きユーザーへの実効反映（レビュー#2・§4.11） | system_admin（OPS）＋対象会社の一般ユーザー（`game_mode_override=null`＝未上書き） | `PATCH .../settings`（`game_mode_default=false`）→ 対象会社ユーザーで `GET /me` | `PATCH`＝`200`＋設定に `game_mode_default=false`（`GET /admin/companies/{id}` にも反映）。**`company_config` 無効化**により、未上書きユーザーの `GET /me` は `game_mode.company_default=false`・**`effective=false`**（会社既定に追従）。個人上書き済みユーザーは個人値を維持（本 TC の主眼は未上書き追従） | B.1／§4.11 |
