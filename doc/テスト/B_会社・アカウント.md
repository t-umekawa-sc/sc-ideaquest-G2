...
| B-TC-035 | api | 発行の Idempotency（同一キー再送は再生・別内容422・横断MW §1.9） | system_admin | `POST .../accounts`（同キー2回→別内容） | 1回目201・2回目 `Idempotency-Replayed:true`・同 account_id／accounts/outbox/mail 各1件（副作用1回）／別内容は422 `idempotency_key_reuse` | §1.9／B.5 |
...
| B-TC-176 | api | ゲームモード会社既定 `game_mode_default` の更新＋永続（レビュー#2・§4.11） | system_admin（OPS）で会社作成 | `GET/PATCH .../settings`（`game_mode_default` を true→false→true） | 既定 `true`。`PATCH`＝`200`＋応答/`GET /admin/companies/{id}` 詳細に反映（`false`）・**リロード相当（再GET）でも永続**・`true` に戻せる（他フラグと独立）。※未上書きユーザー（`game_mode_override=null`）の `GET /me` `effective` が会社既定に追従する点は同機構＝**K-TC-024**（`effective = override ?? company_default`）が担保 | B.1／§4.11 |
...
| B-TC-178 | e2e | 編集で無変更保存＝API を呼ばず info「変更はありません」（保存ボタン統一・成功通知を誤発火しない） | OPS system_admin・ACME-01 詳細（既存 seed アカウントを編集で開く・無編集） | 既存アカウント行「所属・編集」→何も変えず「保存する」 | `info`「変更はありません」トーストが出て成功「アカウントを更新しました」は出ない＝無変更で PATCH を呼ばない（mutation なし＝共有 seed 非破壊・§14 の正パターンへ寄せる） | デザイン標準 §14／SC-92／B.2 |
...
| B-TC-179 | e2e | グループ名の無変更保存＝rename を呼ばず info「変更はありません」（無音にしない・保存ボタン統一） | OPS system_admin・ACME-01 詳細でグループ作成→編集を開く（無編集） | 編集モーダルで何も変えず「保存する」 | `info`「変更はありません」が出て成功「グループ名を更新しました」は出ない＝無変更で rename API を呼ばない（従来は無音）。作成した空グループは後始末で削除 | デザイン標準 §14／B.3.1 |
