...
| K-TC-022 | api | アバター追従アニメ表示 `mascot_follow` の編集＋配信（#20・§4.9 系・account-only） | ACME-01 実アカウントでログイン（既定 true） | `GET /me`／`PATCH /me`（`{mascot_follow:false}`→`true`） | `GET /me` の `account.mascot_follow` は既定 `true`（表示＝現行挙動）。`PATCH` で `false`＝`200`＋応答 `account.mascot_follow=false`・**`accounts.mascot_follow` 更新**（account-only＝`users` へはミラーしない）。`true` に戻せる（他フィールドと独立） | K.2／§4.9 |
| K-TC-023 | unit(front) | 追従アニメの実効表示判定＝純ロジック（抑制優先） | — | `mascotFollowEffective(reduced, follow)`（`impl/frontend/src/features/avatar/follow.ts`） | 実効表示 = `follow AND NOT reduced`。抑制（OS reduce OR reduce_motion）が立てば追従 OFF／抑制でなく follow=true のみ表示／follow=false は常に非表示 | §4.9／#20 |
| K-TC-024 | api | ゲームモード実効値の配信＝`GET /me` の `game_mode`（レビュー#2・§4.11） | ACME-01 実アカウントでログイン（個人上書き未設定＝既定 `override=null`／会社既定 `game_mode_default=true`） | `GET /me` | 応答に `game_mode:{effective, override, company_default}`。既定＝`override=null`・`company_default=true`・**`effective=true`**（`override ?? company_default`）。会社既定を継承していることを確認 | K.1／§4.11 |
| K-TC-025 | api | ゲームモード個人上書き `game_mode_override` の三値編集＋実効再計算（account-only） | ACME-01 実アカウントでログイン（会社既定 true） | `PATCH /me`（`{game_mode_override:false}`→`true`→`null`） | `false`＝`200`＋`game_mode.override=false`・**`effective=false`**（個人優先）・**`accounts.game_mode_override` 更新**（account-only＝`users` へミラーしない）／`true`＝`effective=true`／`null`＝上書きクリアで `override=null`・`effective` は会社既定（`true`）へ復帰。三値以外/allowlist 外は `422` | K.2／§4.11 |
...
| K-TC-026 | e2e | 無変更で保存＝API を呼ばず info「変更はありません」（保存ボタン統一・成功通知を誤発火しない） | OPS でログイン・`/profile`（GET /me 済） | 何も編集せず「保存する」を押す | `info`「変更はありません」トーストが出て、成功「プロフィールを更新しました」は出ない（無変更で更新 API を呼ばない・§14 の正パターンへ寄せる） | デザイン標準 §14 |
