...
| J-TC-141 | api | クエリインジェクション耐性（`q` バインド変数＋PGroonga パースエラー耐性・§2.2③） | 参加クエスト | `?q=` に PGroonga 演算子/特殊文字（`" ( ) \ OR *` や `'; DROP TABLE ideas; --`） | 5xx/例外にならず 200（文字通り検索＝0件 or 一致）。SQL は bind（文字列連結しない）＋**PGroonga のクエリ構文パース失敗（`*`・`((` 等）は該当なしとして安全に握る**（`DBAPIError` を種別ごとに捕捉→rollback→空・application 層）。Groonga ランタイム状態で `pgroonga_query_extract_keywords('*')` が稀に失敗しても 5xx にしない | J.5③／§2.2③／J.0 |

## 参加部署アクセス門番（FR-38 再設計・`can_access_quest`・C.0）

> 全文検索の門番を `can_access_quest` へ統一（2026-09-11・旧「パーティー∩グループ AND」を単一関数に一本化）。**アクセスの都度、現所属で再判定**＝異動で全参加部署を外れたら名指しパーティー員でもクエスト内検索は 404（動的失効）。作成者は別格。

| TC-ID | 階層 | 目的 | 前提 | 操作 | 期待 | 根拠 |
| --- | --- | --- | --- | --- | --- | --- |
| J-TC-142 | api | 動的失効＝全参加部署離脱でクエスト内検索が 404 | G1 のみ所属の非作成者パーティー員→G1 のグループ所属を除去（参加部署 1 件） | `GET /quests/{id}/search?q=...`（当該員） | 404（`can_access_quest` 失効） | C.0／J.0 |
