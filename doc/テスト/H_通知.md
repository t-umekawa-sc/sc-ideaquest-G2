...
| H-TC-211 | e2e | 既読ボタン押下で上へスクロールしない（ユーザー報告の回帰＝sticky 戻るバー直下のボタンで focus 可視化スクロールが走った） | 25件挿入→中ほどへスクロール（`.backlink--float` 直下に行のボタンが来る状態） | バー下端直下の「既読にする」をクリック | `document.activeElement` が当該ボタン（`.n__read`）に**ならない**（onMouseDown で focus 奪取を防止）／`scrollY` が動かない（±4px）／既読化は成立（「既読にする」が1件減る） | SC-02／デザイン標準 §4.10・§4.12 |
...
| H-TC-209 | unit | notificationHref＝通知 ref→遷移先URL 写像（SC-01/SC-02 共有・受入不具合 DFT-E-003 再発防止＝ダッシュボード通知が非リンクだった） | ref 各種／ref 無し | `notificationHref(n)` | chat_message_id+idea_id→`/ideas/{id}/chat`・idea_id→`/ideas/{id}`・achievement_id→`/achievements`・quest_id→`/quests/{id}`・ref 無し→`null` | H.4／SC-02 §4.2 |
| H-TC-210 | unit | markNotificationRead＝既読化を通知一覧へ楽観反映（受入不具合 DFT-E-004 再発防止＝通知クリックで既読化） | 未読2件 | `markNotificationRead(notifs, id)` | 対象 is_read=true・他は不変・unread_count −1（0 下限）／既読 or 不在は同一参照＝冪等（二重デクリメントなし） | H.3／SC-01・SC-02 |
| H-TC-211 | unit | notificationHref＝参加リクエスト受信は当該申請者ダイアログを直接開く（FR-40・通知リンク経路） | type=`join_request_received`・ref に quest_id＋user_id（＝申請者・params 由来） | `notificationHref(n)` | `/quests/{quest_id}?joinreq={user_id}`。user_id 欠落時は `/quests/{quest_id}` にフォールバック | H.4／FR-40／SC-12 |
| H-TC-171 | int | 一覧描画の ref 解決が N+1 にならない担保（`prime_refs` 一括ロード） | 同一宛先に distinct 実績を参照する achievement 通知を K 件（K=2/7）seed | 各 K で `get_notifications` 中の SELECT 数を計測 | SELECT 数が **行数に比例しない**（K=2 と K=7 で同数＝ページ分の ref を IN 一括ロード）。描画結果（body 等）は不変 | H.2／§7 性能（N+1 回避） |
