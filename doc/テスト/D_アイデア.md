...
| D-TC-235 | api | **アイデアの関連情報**（`GET /ideas/{id}/related-info`・FR-41・SC-22 概要直下 strip）＝一致度 `score` 降順・`rejected`/archived 除外・**manual は `linked_by`（関連付けた人）** | published アイデア＋`info_links` を直接 seed（auto`supporting`score0.80／manual`refuting`／rejected auto／archived 情報） | `GET /ideas/{id}/related-info` | 200・auto/manual を含み rejected/archived 除外・auto(0.80) が manual(NULL) より前・auto は `linked_by=None`／manual は `kind=refuting`・`linked_by.user_id=作成者` | D（related-info）／N.1／§5.35 |
...
> **§4.7 の入力検証は主ボタン押下で到達**（2026-09-16 是正・ユーザー指摘）＝主ボタン「投稿する／変更を保存」は**常に押せ**（旧 `disabled={!canSave}` を撤廃＝§4.7 逸脱だった・評価/クエストと統一）、押下時に client `validate()` が走り不足を**上部サマリ＋インライン（aria-invalid）**に出す。下書き保存は検証スキップ（loose）。サーバエラー（完了クエスト編集の 409・公開状態機械の 409 等）経由の 3 チャネル（サマリ scroll＋足元ヒント＋sticky スナックバー）は **D-TC-216** で担保。押下時 client 検証（活性＋インライン）は D-TC-204 で担保する。
...
| D-TC-204 | e2e | 主ボタンは常に押せ、押下で §4.7 検証（評価/クエストと統一・旧 disabled ガード撤廃・ユーザー指摘） | 登録フルページ | 3 必須空で「投稿する」押下／件名を空 blur | 「投稿する」は**常に活性**・押下で `aria-invalid`＋インライン「件名は必須です。」／件名 blur でも `aria-invalid`／3 必須充足でも活性 | §4.7／SC-21 |
...
| D-TC-214 | e2e | SC-22 完了クエストは投票を事前無効化・新規フォローは info（無効化しない） | recruiting→…→completed に遷移した published アイデア | `/ideas/{id}` を表示 | 「▲ 賛成」「▼ 反対」が `disabled`（凍結理由 title）・「☆ フォロー」は **enabled**（title で理由・押下で info＝解除のみ可・memory: completed-quest-freeze-ui-standard）。サーバー 409 も権威 | D.5/D.6／SC-22 §4.5／C.5 |
...
| D-TC-216 | e2e | SC-21 サーバエラー（完了クエスト編集 409）で §4.7 の 3 チャネルが発火 | recruiting クエスト＋published アイデア。**recruiting のうちに編集フォームを開き**（完了編集は事前無効化のため）、件名変更後に **API で `completed` へ遷移**（`/quests/{id}/transition`）| `/ideas/{id}`→「編集」→件名を変更→（API で completed）→「変更を保存」（PATCH が 409 invalid_state）| `mapServerErrors`→conflict で ① 上部サマリ `.form-summary`「現在の状態では実行できません。」・② 足元ヒント `.form-footer-error`「⚠ 入力エラーがあります。…」・③ エラースナックバー `.snackbar--error`（2026-09-06 改定で **timer あり＝自動消滅**・持続は①②が担う）の 3 チャネルが出る | §4.7／D.0/C.5／SC-21 |
...
| D-TC-218 | e2e | SC-21 編集モードで既存添付の一覧＋**ステージ削除（保存で確定）** | API で published アイデア作成＋添付1件アップロード | `/ideas/{id}`→「編集」→保存済み添付が一覧に出る→× で**削除予定にマーク**（`.attach.is-removing`・「元に戻す」）→「変更を保存」 | 編集フォームに保存済み添付名が出る（`getIdea.attachments`）・× で**削除予定にマーク**（即時削除しない・確認ダイアログ無し）／「元に戻す」で解除／**保存で確定**（`deleteAttachment`）→`GET /ideas/{id}` の `attachments` が空・**版が増え**、更新履歴の差分に `attachments`（📎 添付）削除が出る（D-TC-145）。キャンセルなら無変更（削除も取り消し） | D.3／D.4／SC-21 |

> **削除 UI の置き場所（SC-22 §4.3 準拠）**＝SC-22 の関連資料は「一覧＋ダウンロード」のみ（削除ボタンは無い・0件なら非表示）。添付の**削除は SC-21 フォームの添付チップ（×）**で行い、EP は `DELETE /ideas/{id}/attachments/{aid}`（api の D-TC-134 で担保）。SC-21 編集モードでの**既存添付の一覧/削除 UI は実装済み（D-TC-218）**＝`getIdea.attachments` を読み込み `.attach` 行で表示し、× は**削除予定にマーク（ステージ）→「変更を保存」で確定**（新規追加のアップロードと挙動を揃える・キャンセルなら無変更）。**添付の追加/削除は版に記録され更新履歴の差分に出る（D-TC-145・`_content_snapshot` に添付名一覧を含める）**＝フォロワーが削除に気づける（版記録＋idea_updated 通知）。投稿前の未アップロード添付（新規追加分）の × は従来どおりクライアント除去。
>
> **版の作られ方（設計B・2026-09-14 確定・frontend `IdeaForm.persist`）**＝**1保存1版**。(1) **無変更保存では版を作らない**＝編集開始時の内容（`originalRef`）と比較し、本文いずれも未変更かつ添付（追加/削除）も無い場合は `updateIdea` を呼ばない（空更新で版を進めない）。(2) **作成時に添付したファイルは初版(rev1)に記録**＝公開作成で添付がある場合は `draft作成→添付アップロード→publish` の順にし、publish 時のスナップショットに添付を含める（後続版の差分が「(なし)→全件」に誤表示されない）。(3) 編集保存は「添付適用→`updateIdea`」の順＝1版に本文＋添付変更をまとめて記録。
...
> クエストの門番を `can_access_quest`（quests repository・門番の単一ソース）へ統一（2026-09-11）。アイデア一覧/詳細も「有効パーティー員 かつ（参加部署 0 件なら条件なし／1 件以上なら現在いずれかに有効所属）」で参照可。**作成者は別格**。**アクセスの都度、現所属で再判定**＝異動で全参加部署を外れたら名指しパーティー員でも 404（動的失効）。

| TC-ID | 階層 | 目的 | 前提 | 操作 | 期待 | 根拠 |
| --- | --- | --- | --- | --- | --- | --- |
| D-TC-222 | api | 動的失効＝全参加部署離脱でアイデア一覧/詳細が 404 | G1 のみ所属の非作成者パーティー員→G1 のグループ所属を除去（参加部署 1 件のクエスト） | `GET /quests/{id}/ideas`／`GET /ideas/{id}`（当該員） | いずれも 404（`can_access_quest` 失効・存在秘匿） | C.0／D.0 |
| D-TC-223 | api | 版追加→**投票者∪フォロワー−編集者**に idea_updated 生成（生成テスト新設） | user 著者/編集者・other=フォロワー(非投票)・voter=投票者(非フォロワー・別ユーザー)・編集者も投票 | 公開中アイデアを `PATCH`(本文変更) | 版が rev2 へ／**フォロワー(other)・投票者(voter)に各 `idea_updated` 1件**・編集者(user)は0（投票者でも除外）・`params.revision`=新版・`ref_idea_revision_id` 設定 | D.4／FR-34／H.0 |
