...
- レート制限（ADR-0001 §2.6・**ログイン**の 429）＝**A-TC-050 で実装済み**（§5 の表・第一層＝(IP+login_id) 固定窓・上限超過で 429 rate_limited・ロックとは別層）。
...
| A-TC-050 | api | ログインのレート制限（第一層・429） | 固定 IP・実アカウント（Redis クリーン） | 上限（`login_rate_limit_max`＝既定10）まで正PWで login →もう1回 | 上限内は `200`（成功でも枠を消費）／上限超過は **`429` `rate_limited`**（ロックの 401/423 とは別層・成功では枠がリセットされない） | ADR-0001 §2.6／`check_login_rate_limit` |
