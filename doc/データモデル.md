...
**DB 跨ぎの参照は物理 FK を張れない**（別インスタンスのため）。管理 DB の `accounts` と 会社 DB の `users` は **`account_id`（UUID）で論理的に対応付け**る。整合はアプリ層で担保し、**認証系属性（`login_id`/`email`/`status`）は `accounts` を源泉に `users` へ一方向ミラー**する（アウトボックス＝§4.7 `account_sync_outbox`・冪等/結果整合）。これによりユーザ一覧は会社 DB 単独で描画できる（§5.3・§8-①）。
...
- **外部キー**: 会社 DB 内は物理 FK を張る。**DB 跨ぎ**（accounts↔users, account_quest_groups↔quest_groups）は **論理参照**（FK なし・アプリ整合）。**FK カラムの命名規則は §2.2** に定める。
...
- **DB 跨ぎの参照**（`accounts`↔`users`、`account_quest_groups`↔`quest_groups`）は物理 FK を張らない **論理参照**。カラム名は同一規則に従い、定義表に「論理参照→…」と明記する。
...
erDiagram
  ...
  account_quest_groups["account_quest_groups（アカウント×クエストグループ所属）"] {
    uuid id PK "ID"
    uuid account_id FK "アカウントID"
    uuid quest_group_id "クエストグループID（論理参照）"
  }
...
- **会社 DB `users` へのミラー（源泉＝本テーブル）**: `login_id` / `email` / `status` は会社 DB `users` に**一方向でミラー複製**し、ユーザ一覧を会社 DB 単独で描画できるようにする（API 往復を避ける）。**源泉（source of truth）は本 `accounts`** で一意検証もここで行い、変更は §4.7 `account_sync_outbox` 経由で会社 DB へ反映する。詳細は §8-①。
...
### 4.7 account_sync_outbox（会社DB同期アウトボックス）
...
- **`login_id`/`email`/`status` はミラー列**（源泉＝管理DB `accounts`・一方向反映）。ユーザ一覧は本テーブルのみで描画し（`WHERE status='active'`）、管理DBへの API 往復を行わない。**変更契機＝①管理者のアカウント発行/編集/無効化/PWリセット（SC-90/92）②本人のプロフィール編集（`login_id`/`email`）**。いずれも管理DB `accounts` を先に更新（一意検証込み）し、§4.7 `account_sync_outbox` 経由で本テーブルへ反映（冪等・再試行）。会社DB 側でこれらを直接編集しない（ミラーの上書き防止）。
...
1. **管理 DB ↔ 会社 DB の users 同期 ＝ 決定（2026-07-20）**: `accounts.login_id`/`email`/`status` を会社 DB `users` に**一方向ミラー**し、ユーザ一覧は会社 DB 単独で描画（`WHERE status='active'`・API 往復なし）。**源泉＝`accounts`**（一意検証もここ。`login_id`・`email` は**会社内で一意**）。反映は §4.7 `account_sync_outbox`＋ワーカで冪等・結果整合。変更契機＝管理者操作（SC-90/92）＋本人のプロフィール編集（`login_id`/`email`）。**残TBD**: **クエストグループ所属の配置**（`account_quest_groups` を管理 DB に置くか、会社 DB の `quest_group_members` に一本化するか）はログイン時参照範囲判定との兼ね合いで別途確定（§4.5 代替案）。
