...
- **冪等キー（`Idempotency-Key`）**: 非冪等 POST のうち二重送信で不整合が出るもの（アイデア投稿・クエスト作成・購入・評価確定・魔法解放 等）に付与。サーバーは同キーの再送に対し**最初の結果を再生**する（`idempotency_replayed`）。
  ...
  - **実装（2026-08）**: 横断ミドルウェア `impl/backend/app/core/idempotency.py`（`main.py` に `add_request_id` の内側で登録）＝**POST かつ `Idempotency-Key` ヘッダ有りのみ作動**（未付与 POST は素通し）。フロントは `impl/frontend/src/lib/api/client.ts` の `idempotencyHeader()` で送出（**購入・魔法解放は配線済み**／アイデア投稿・クエスト作成・評価確定は順次配線）。テスト＝[`../テスト/セキュリティ横断.md`](../テスト/セキュリティ横断.md) SEC-TC-041〜044。
