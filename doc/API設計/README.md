...
- **フィルタ/検索/並び替え**は `.list-toolbar`（各 SC の一覧標準）に対応するクエリ: `?q=`（検索）・`?status=`・`?sort=`（例 `sort=-created_at`＝降順）・画面固有フィルタ（例 クエスト一覧 `?group_id=`、アイデア一覧 `?rating=`）。詳細な体系（複数ソート・項目別フィルタ・CSV・ピン）は直下の **DataTable クエリ契約** に従う。

#### 1.8.1 DataTable クエリ契約（一覧の操作標準のサーバー委譲・決定 2026-08-17）

> [`画面設計/デザイン標準.md §4.5`](../画面設計/デザイン標準.md)「一覧の操作標準」は、テーブル一覧に**横断検索・複数キーソート・項目別フィルタ・番号ページャ・CSV エクスポート・行固定（ピン）**を一括付与する会社標準で、その**サーバー処理前提**（並びの最終決定・件数・CSV 生成・ピンのページ跨ぎはサーバー）を各一覧 API に要求する。本節はその要求を満たす**共通クエリ契約**を定める＝フロント（`impl/.../components/ui/DataTable.tsx` の `computeRows` パイプライン: 検索→項目別フィルタ→複数ソート→ピン分離→ページ）を**無改造でサーバー委譲**できる形。**モックは全件クライアント保持で本契約の挙動を再現**する（DoD＝モック一致）。
>
> **なぜ本契約が要るか**: 管理系一覧（会社・アカウント＝小規模）は当面クライアント全件処理で足りるが、**アイデア一覧（SC-12）・クエスト一覧（SC-10）は PGroonga 全文検索がサーバー専用・件数無制限・カーソルページング**でクライアント処理が不適（§1.11・C.1・D.1）。それらに一覧の操作標準を適用するには、並び/絞込/件数/CSV/ピンをサーバーで解決する本契約が前提になる。

- **① 複数ソート（`?sort=` の多重化）**: `?sort=-created_at,name` ＝**カンマ区切り・左が最優先・`-` 接頭辞で降順**（既存の単一 `sort=-created_at` 記法の自然な拡張）。フロントの複数キー（`advSort`。無ければ単一列 `simpleSort`）をそのまま連結する。**ソート可能キーは EP ごとにホワイトリスト**で定義し、未知キーは `422 validation_error`（`errors[].field="sort"`）＝任意列ソートによる情報漏れ・注入を防ぐ。カーソル方式では**カーソルが全ソートキーのタプルを内包する不透明値**（複数ソートでも決定的順序を保証・末尾に `id` を暗黙付与して一意化）。
- **② 項目別フィルタ（横断 `?q=` と AND 結合）**: フロントの `FilterCond` 4 型に 1:1 対応する**フラットな命名クエリ**（`filter[field][op]=` のブラケット式は用いない＝既存 `status=`/`group_id=` と地続き）。
  - text（含む）＝`?<field>_contains=<文字列>`
  - enum（複数選択）＝`?<field>=a,b`（CSV。既存の単値 `status=` を多値化）
  - number（範囲）＝`?<field>_min=<n>` / `?<field>_max=<n>`
  - date（範囲・ISO8601）＝`?<field>_from=<d>` / `?<field>_to=<d>`
  - **フィルタ可能フィールドも EP ごとにホワイトリスト**。未知フィールド/型不一致は `422 validation_error`。横断検索 `?q=` は各 EP が定める対象列の**部分一致**（アイデア/チャット等の全文検索は §1.11 の PGroonga EP に委譲＝一覧 EP の `q` は簡易絞り）。
- **③ CSV エクスポート（同一 EP の `?format=csv`）**: 別 EP を新設せず、一覧 EP に `?format=csv` を付す＝**同一の絞込・ソートを適用した全件**（ページング無視）を返す。`Content-Type: text/csv; charset=utf-8`（**UTF-8 BOM 付・Excel 互換**）＋`Content-Disposition: attachment; filename=...`。**表示中の列・列順**は `?columns=name,code,status`（フロントの表示状態）で受ける（デザイン標準 §4.5⑦）。**管理系のエクスポートは監査ログ対象**（B.1/B.2 の権限変更履歴と同じ `system_audit_logs` へ「エクスポート実行」を記録）。大量時の非同期化（ジョブ化）は将来拡張。
- **④ 行固定（ピン）のページ跨ぎ（`?pin_ids=`）**: `?pin_ids=<id>,<id>` ＝**絞込やページで母集合から外れても、これらの行は必ず解決して返す**（フロントは固定セクションを常時先頭描画・件数/ページャは非固定母集合が対象）。件数上限は `maxPins`（既定 5）。**ピン ID の保持先は当面クライアント（`localStorage`・画面×列単位）**＝クライアントが毎リクエスト送る。サーバー側ユーザー設定への格上げは将来（デザイン標準 §4.5⑨「将来はユーザー設定へ格上げ」に整合）。
- **⑤ 番号ページャ（offset）とカーソルの使い分け**: 本契約は §1.8 の使い分けをそのまま踏襲する。
  - **番号ページャ（`« ‹ 1 … › »`）＝オフセット型**（`?page=`/`?per_page=`＋`page_info.total`）。総件数バッジが要る**管理系一覧（会社・アカウント）**はこちら。
...
- **冪等キー（`Idempotency-Key`）**: 非冪等 POST のうち二重送信で不整合が出るもの（アイデア投稿・クエスト作成・購入・評価確定・魔法解放 等）に付与。サーバーは同キーの再送に対し**最初の結果を再生**する（`idempotency_replayed`）。
  ...
  - **実装（2026-08）**: 横断ミドルウェア `impl/backend/app/core/idempotency.py`（`main.py` に `add_request_id` の内側で登録）＝**POST かつ `Idempotency-Key` ヘッダ有りのみ作動**（未付与 POST は素通し）。フロントは `impl/frontend/src/lib/api/client.ts` の `idempotencyHeader()` で送出（**購入・魔法解放・アイデア投稿(作成/公開)・クエスト作成(作成/公開) 配線済み**／評価確定は `PUT`＝HTTP 冪等でヘッダ不要）。テスト＝[`../テスト/セキュリティ横断.md`](../テスト/セキュリティ横断.md) SEC-TC-041〜044。
...
- **多重化とトピック購読**: 1 本の WS 上でメッセージにトピックを付与。
  ...
  - メッセージ形（サーバー→クライアント）: `{ "topic": "...", "type": "chat.message.created|chat.reaction.added|chat.reaction.removed|chat.message.updated|chat.message.deleted|notification.created|notification.unread_count", "data": {...}, "id": "<event_id>" }`（canonical カタログ＝L.3）。**`type` は機械可読**、`data` は該当リソースの表示用ペイロード。
...
- **プレゼンス/タイピング表示は将来**（同 WS 上で拡張可能な設計にしておく）。**外部通知（メール等）も将来**（i18n の通知テンプレはデータモデル §8-⑬/コーディング規約 §2.1）。
...
| P | コンセプト創造・検証（ISO56002 ②③段） | SC-xx（新規） | テナント | 🟡 ドラフト | [`P_コンセプト.md`](./P_コンセプト.md) |
...
### I. ダッシュボード集約（SC-01）＝詳細確定
→ **[`I_ダッシュボード集約.md`](./I_ダッシュボード集約.md)**。決定＝**①取得方式＝集約1本 `GET /dashboard`**（SC-01 §10 の未決を決定・ランディングの1往復優先／不採用＝分割並列＝横断リストEPの乱立）**②I は「読取合成の殻」＝新しい業務ロジックを持たない**（匿名化/visibility/権限/ランキングスコアは各ドメインの純粋関数・repository が権威）**③横断クエリ（全クエスト/全アイデア跨ぎの下書きアイデア/評価・未投票・フォロー中）は所有ドメイン（D/F）の repository に read を追加して I が合成し、別個の REST エンドポイントは新設しない**（I.3・規約 §3.1/§3.5-(2)）**④ヒーロー残高（Lv/XP/コイン/SP）は残高の正準＝K の `GET /me`。I も `GET /dashboard` に同梱**（両立・別用途＝K.1／G.0 と整合）。レスポンス＝`{hero, drafts[], unvoted_ideas[], quests[], followed_ideas[], weekly_ranking, notifications, roles, login_bonus?}`。合成元＝C（参加中/下書きクエスト）・D（下書き/未投票/フォロー中アイデア）・F（下書き評価）・G（週間ランキング・残高）・H（最近の通知）・A（`login_bonus` のワンショット）。
...
### N. 情報インプット（外部情報の知識レイヤ・テナント）＝設計ドラフト（2026-09-18）
→ **[`N_情報インプット.md`](./N_情報インプット.md)**。決定＝**①会社横断の知識プール**（クエスト門番でなく会社内 active 全員が閲覧）＋**段階権限**（低摩擦登録=全員／属性付与・判定・アーカイブ=`info_curator`／`info_curator` 付与=会社アカウント管理者）**②取り込みは手動貼付のみ=SSRF 対象外・`source_url` は http(s) のみ・本文は表示時サニタイズ**（§N.7）**③一覧は DataTable サーバー委譲**（`GET /info-items`・分類 enum フィルタ・全文=PGroonga・ワードクラウド `GET /info-items/word-cloud`）**④動的リンク**＝情報保存/成果物保存の内部トリガで類似度（janome＋TF-IDF・`info_links.score` 事前計算）→ auto リンク（既定 `related`・通知なし）／人が種別変更（関連↔裏付け↔反証）・手動追加・棄却**⑤「根底を揺さぶる」発火は per-link `refuting`（manual）のみ**（`impact_class=threat` は初期サジェスト・自動発火しない）→ 作成者+評価者へ通知+要再評価**⑥成果物→関連情報は各ドメイン read に委譲**（`GET /{artifact}/related-info`・N に横断 EP を増やさない）**⑦MVP=キーワード/TF-IDF・Phase2=埋め込み/矛盾自動検出/enum 会社拡張/重複統合**。※テスト接頭辞＝**N-TC**（M は共通シェル・ナビで使用済み）。データモデル §5.33-5.37。要件＝FR-41。

### P. コンセプト創造・検証（ISO56002 ②③段・テナント）＝設計ドラフト（2026-09-24）
→ **[`P_コンセプト.md`](./P_コンセプト.md)**。決定＝**①ドメインレター P**（A〜O 使用済み・テスト接頭辞 P-TC）**②門番＝クエストベース**（パーティー所属 AND クエストグループ所属＝C.0/D.0 同一・コンセプト/前提はクエスト配下）**③コンセプト本体は D アイデアと同型**（作成=作成者+owner/quest_admin／`draft→active→archived`／選定 `is_selected`〔owner/quest_admin・複数可〕と判定 `decision`〔go/pivot/kill〕は別操作）**④前提＝クエスト単位の検証プール**（第一級・編集はプール所有=owner/quest_admin／検証イベントは追記型・`refuted` で波及）**⑤コンセプト↔前提は M:N＋重要度**（リンク単位・エビデンスは前提側の単一ソース）**⑥投票＋評価の2層**＝投票（賛成/反対・D同型・`vote` 権限・**XP+5 参加促進**）＋評価（F同型・中核5必須＋補助3任意＝ISO §8.3.3 b・評価者の Go/Pivot/Kill 推奨・評価/選定の XP・コインは実装時）**⑦チャットは P にスコープ単位EP**（`/concept-chat-scopes/{id}/messages`・機構は E 再利用・E は無改修／リアクションは E の `/chat-messages/{id}/reactions` 流用／RT は L 拡張）**⑧反証波及**＝前提の refuted → 全リンク先 stale＋作成者/評価者へ通知（H）＋情報リンク反証（N）と同経路**⑨情報リンクは read 委譲**（横断EP増やさない）。付随＝関連リンク対象ピッカーへ concepts/assumptions 追加（N `search_link_candidates`）。データモデル §5.38-5.45。要件＝FR-42。
