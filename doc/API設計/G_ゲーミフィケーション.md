...
## G.5.1 アクティビティフィード（時系列・SC-03 個人／SC-12 クエスト内／SC-01 チーム・2026-08-19 決定）

> **なぜ**＝XP/コイン/SP の付与・消費はすべて元帳 `activities`（データモデル §5.27）に記録されるため、
> 「時系列アクティビティ」は**元帳の絞り込み表示**で表現でき、専用テーブルもスケジューラも要らない（DRY・
> ランキング G.5 が `activities.quest_id` を集計軸に使うのと同じ思想）。ユーザー選択（2026-08-19）で採用。

対象画面と絞り込み軸（すべて `activities` を源泉・新テーブル不要）:

| メソッド/パス | 画面 | 対象範囲 | リクエスト | レスポンス |
| --- | --- | --- | --- | --- |
| `GET /me/activities` | SC-03 個人履歴 | **自分の全 activity**（全種別・私的行動含む） | `kind?`・`period?`・`limit`/`cursor`（§1.8） | `data`=活動行＋`page_info`（**実装済み**・K/me ルータ） |
| `GET /quests/{quest_id}/activities` | SC-12 クエスト内 | 当該クエストの**参加メンバー**の活動（`activities.quest_id={id}`・**公開種別のみ**） | `limit`/`cursor` | `data`=活動行（`actor`〔id/氏名/アバター/level〕＋種別＋`ref`＋`created_at`）＋`page_info` |
| `GET /me/feed` | SC-01 チーム | **自分が有効参加している全クエスト**（横断）のメンバー活動（`quest_id ∈ 自分の参加クエスト`・**公開種別のみ**） | `limit`/`cursor` | 同上（`quest`〔id/件名〕を各行に付す） |

- **公開種別（他者フィードのプライバシー・初期値＝調整可）**: 他人に見せるのは**成果系のみ**＝`idea_post`（投稿）／`selection`（選定）／`achievement_reward`（実績）／`levelup_sp`（レベルアップ）。**出さない**＝`vote`（投票匿名化 FR-23 と衝突）・`chat`（ノイズ）・`login`・`shop_purchase`(コイン消費)・`spell_unlock`(SP消費)・`evaluation`/`evaluation_coin`（採点・獲得額は評価公開範囲 F に従属＝当面フィード非対象）。**自分自身の履歴（SC-03）は全種別**を出す（プライバシーは自分に及ばない）。
