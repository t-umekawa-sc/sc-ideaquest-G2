...
- **ブルートフォース閾値**：**MFA の OTP 連続失敗上限（5）・resend クールダウン（30秒）・OTP 桁数/TTL（6桁/10分）は確定＝[`../ADR/ADR-0004_MFA・信頼端末基本パラメータ.md`](../ADR/ADR-0004_MFA・信頼端末基本パラメータ.md) §2.1**（env）。ログイン失敗の**アカウント一時ロック**は確定＝[`../ADR/ADR-0005_アカウント一時ロック.md`](../ADR/ADR-0005_アカウント一時ロック.md)（**(IP+login_id) 単位・5回→15分・ロック中も一律 401＝残時間を返さない・発火時に本人へ通知メール（1通/60分/account）・PW 再設定成功で即解除**・env・第二層）。※レート制限・ロックの「IP」は**実クライアント IP**を用いる（リバプロ/BFF 配下での確定方式＝[`../ADR/ADR-0006_クライアントIPの確定.md`](../ADR/ADR-0006_クライアントIPの確定.md)・本番の保証は [`../本番デプロイ要件.md`](../本番デプロイ要件.md)）。※**パスワード再設定要求（`password-setup/request`）のレート制限**は確定＝[`../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md`](../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md) §2.3（(IP＋company_code＋login_id) 5回/10分・**超過時も `202` 維持**で列挙耐性）。
- **PW ポリシー具体値**：確定＝[`../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md`](../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md) §2.2（**最低8文字＋英字1＋数字1**・§A.9-④）。※漏えい済み/よく使われる PW 拒否リストは後続スライスへ委譲（ADR-0002 §2.2）。
- **設定リンクトークン方式**：確定＝[`../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md`](../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md) §2.1（CSPRNG 32B base64url・**SHA-256 ハッシュ保存**・72h・単回・`request` で旧チャレンジ失効）。
- **監査ログ**：保存期間・出力先（方針＝§A.9-⑥）。
- メール送信基盤（dev=MailHog／prod=SMTP）＝[`../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md`](../ADR/ADR-0002_初回・再設定パスワード基本パラメータ.md) §2.5（送信ポートで抽象化・テストはフェイク送信）。接続設定の実値は `.env`。
