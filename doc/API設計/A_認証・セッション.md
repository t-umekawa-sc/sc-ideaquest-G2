...
- **管理者再設定との関係**: B.2/B.4 の `password-reset`（管理者起点）と本 `request`（本人起点）は**同じ `password_setup` リンクを発行**する等価な入口。監査上は起点（操作者）で区別（§A.9-⑥）。
