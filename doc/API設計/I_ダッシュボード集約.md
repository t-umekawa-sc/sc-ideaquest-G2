...
- **`login_bonus?`**（デイリーログインボーナス演出・SC-01 §4/§6・任意）
  **その日の日次ログイン XP（`reason=login`）が当日初回に付与されていれば**、初回ダッシュボード取得時に一度だけ返す（多重表示を防ぐため返却後にワンショットフラグを消費）。**付与契機は「新しい暦日（JST）の最初の認証済みリクエスト」＝ログイン成功に限らず持続セッションでも毎日成立**（正＝A.1／G.6）。XP 付与自体はセッション解決時に確定済み（1日1回・日次上限＝A/G）で、**ここは演出データのみ**。形＝`{xp, message?, levelup?}`（`levelup` 詳細演出は SC-01 §10 TBD）。
...
- **実装で確定（2026-08-26・MVP）**: 表示件数＝**通知 5／未投票・参加中・フォロー各 6／下書き全件**（少数想定）。並び＝下書き/フォロー＝更新降順・未投票/参加中＝締切近い順（PG ASC は NULLS LAST）。**部分失敗＝パネル単位 best-effort**（合成失敗のパネルは `null`/空配列・全体は 200・認証/テナント解決失敗のみ 401）。`login_bonus` のワンショット＝**Redis フラグ `dashboard:login_bonus:{user_id}`**（A のログイン成功で `grant_daily_login` が実付与した時に `mark`・I が `GET /dashboard` で `consume`＝GETDEL で1回だけ返す）。横断 read は D/F の repository（`list_draft_ideas_by_author`/`list_unvoted_published_ideas`/`list_followed_ideas`／`list_draft_evaluations_by_evaluator`）＋C `list_member_quest_ids`。実装＝`app/tenant/dashboard/`（application/router/login_bonus）。リッチパネル（週間ランキング/参加中クエスト/通知）は各ドメイン application を再利用。フロント＝`features/dashboard/api.ts` getDashboard＋`DashboardView` 実接続（クイック投票＝POST /ideas/{id}/vote・フォロー解除＝D follow EP・login_bonus トースト）。
- **残 TBD（軽微・将来）**: 各 read の**キャッシュ/タイムアウト**・**並列実行**（現状は逐次・best-effort）／**レベルアップ演出**（SC-01 §10）／通知ベルの簡易ドロップダウン化（SC-01 §10）／実績サマリ（直近バッジ）を載せるか（SC-01 §10・現状は非搭載）／3D アバターの表示方法（静止画キャッシュ or 軽量3D・SC-01 §10）／未投票/フォローの `category`・`comment_count` 等の付加情報（現状は最小 DTO）。
