...
- **スコープ境界（2026-09-24 確定・[コンセプト機能 ISO56002 再設計](../設計ドラフト/コンセプト機能_ISO56002_再設計.md) §1.1）**＝コンセプト/前提は**クエスト内**（`quest_id` はホームスコープ／検証プールのスコープ）。**クエストをまたぐコンセプトは直接作らない**（門番の単一モデルを崩さない）＝横断は**「収束クエスト」**（複数ソース取り込みの後続クエスト・quest merge は非採用）で実現。**④ソリューション開発・⑤導入はクエスト外＝PM/WBS の領域**＝本ドメインは go 判定コンセプトの産出まで（④⑤ EP は持たない）。
