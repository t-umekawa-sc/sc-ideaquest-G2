...
対象＝**ISO 56001 ②コンセプトの創造＋③コンセプトの検証**（原本 §8.3.3/§8.3.4）を担う新ドメイン。現行アプリのアイデア段（①機会の特定＋選別）の**次段**を、**クエスト（＝1イノベーション・イニシアチブ／検証プールの単位）配下**に積む。要件＝**FR-42**（[要件定義 README](../要件定義/README.md)）／正本＝[コンセプト機能 ISO56001 再設計](../設計ドラフト/コンセプト機能_ISO56001_再設計.md)。データモデル §5.38〜5.45・§3（`concept_status`/`concept_decision`/`assumption_verdict`/`concept_criticality`/`concept_eval_aspect`/`concept_chat_scope_kind`）。すべて**テナントAPI**（会社DB）。テスト接頭辞＝**P-TC**。
...
- **XP＝コンセプト投票のみ第一版で付与**（XP+5・各コンセプト初回・参加促進＝2026-09-24 決定・P.5b）。**評価/選定/投稿の XP・コインは実装時に判断**（データモデル §5.43 の申し送り）。
...
| コンセプトへの投票（賛成/反対） | `vote` 権限（新規参加者の既定権限） | D アイデア投票と同型（P.5b） |
...
- **スコープ境界（2026-09-24 確定・[コンセプト機能 ISO56001 再設計](../設計ドラフト/コンセプト機能_ISO56001_再設計.md) §1.1）**＝コンセプト/前提は**クエスト内**（`quest_id` はホームスコープ／検証プールのスコープ）。**クエストをまたぐコンセプトは直接作らない**（門番の単一モデルを崩さない）＝横断は**「収束クエスト」**（複数ソース取り込みの後続クエスト・quest merge は非採用）で実現。**④ソリューション開発・⑤導入はクエスト外＝PM/WBS の領域**＝本ドメインは go 判定コンセプトの産出まで（④⑤ EP は持たない）。
...
- **検証実施日（`validated_on`）・規模（`scale`）を残す**＝エビデンスの古さ・強さの材料（§3.5・ISO56001 §9.1 継続的モニタリング）。
...
アイデア投票（[`D_アイデア・添付・版・投票・フォロー.md`](./D_アイデア・添付・版・投票・フォロー.md) 投票 EP）と同型。パーティー員がコンセプトに賛成/反対（1人1票・変更/取消可）。**評価者の正式評価（P.5）とは別層＝競合コンセプトの母集団の選好シグナル**。

| メソッド/パス | 概要 | リクエスト | レスポンス |
| --- | --- | --- | --- |
| `POST /concepts/{concept_id}/vote` | 投票（登録/切替・冪等 upsert） | `{type: approve\|oppose}` | 集計（`approve`/`oppose` 数・`my_vote`）。**XP+5 は各コンセプト初回のみ・日次上限（§7・アイデア投票と同ルール）** |
| `DELETE /concepts/{concept_id}/vote` | 取消 | — | 更新後の集計 |

- **冪等・トグル稼ぎ防止**＝アイデア投票の冪等ルール踏襲（`activities.reason='concept_vote'`・`ref_type=concepts`・XP は初回のみ・切替/取消で追加なし・付与検証は G 委譲）。
- 記名/匿名は `companies.vote_anonymized`（表示のみ制御・データは常に保持）。完了（`completed`）で凍結（P.0）。
