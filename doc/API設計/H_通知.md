...
- **`security_*` はオプトアウト不可**（A.9-⑧・将来の種別 ON/OFF 対象外）。`security_password_changed` のメール実送信は A の認証フロー（H は会社DBの通知行を担当）。**`security_password_changed` は A（初回設定/再設定）に加え K（プロフィールでの自己PW変更・A.9-⑧(b)）も発火元**＝どちらの application も post-commit で `notify()` を呼ぶ（K は未着手・ここで契約を先取り）。
