...
| `POST /admin/companies` | 会社を新規作成（SC-91） | ボディ: `name`,`company_code`,`db_identifier`,`color`,`icon_image_path?` | 作成された会社（**`status=suspended`＝停止**で返す＝作成時点は会社DB未整備）。`company_code` は大文字正規化＋一意検証／DBプロビジョニングは下記 `provision` で実施・完了後に `active`（有効）化 |
| `POST /admin/companies/{company_id}/provision` | 会社DBをプロビジョニング（SC-92「会社DB」・2026-08-22 追加） | パス: `company_id` | 更新後の会社詳細（`status=active`）。**会社DB作成→マイグレーション head→会社DB `users` ミラー seed→`active` 化**を実行。**冪等**（CREATE DATABASE は存在時スキップ／alembic head 済みは no-op／ミラーは無い分のみ／status は active 確定）。system_admin 専用・Origin/CSRF。実体＝`scripts.bootstrap` の `create_database`/`migrate_company` を流用 |
...
| `PATCH /admin/companies/{company_id}/settings` | 会社設定フラグを更新（SC-92） | パス: `company_id`／ボディ: `vote_anonymized`,`hide_voters_from_managers`,`mfa_required`,`game_mode_default` | 更新後の設定フラグ。**`vote_anonymized=false`（記名）時は `hide_voters_from_managers` を無効化して保存**（サーバーで整合）。`game_mode_default`＝会社全体のゲーム層UIの既定 ON/OFF（レビュー#2・下記） |
...
- **`game_mode_default`（ゲームモード会社既定・レビュー#2）**: 会社全体のゲーム層UI（演出・ショップ/きせかえ/魔法・実績・ランキング・ゲーム系通知・チャット魔法）の**既定** ON/OFF。**実効値は個人上書き優先**＝各ユーザーの表示は `accounts.game_mode_override ?? companies.game_mode_default`（`GET /me` の `game_mode.effective`＝API設計 K.1）。**個人が未指定（NULL）の間だけ本既定に追従**するため、本既定を切り替えると `company_config` 無効化により**未上書きユーザーへ次リクエストから即時反映**（上書き済みユーザーは個人値を維持）。**ゲームのロジック（XP/コイン付与・レベル計算）は本フラグと無関係に据え置き**＝UI 表示の gating のみ（レビュー#2 §6）。
...
- **プロビジョニング**は `POST /admin/companies/{id}/provision`（SC-92「会社DB」・2026-08-22 追加）で管理 EP 化（DB作成→マイグレーション→ミラー→active 化・冪等）。**停止・削除・データ退避**は引き続き MVP 手動（§8-⑫）。退会フローは将来（SC-91/92 §9）。
...
| メソッド/パス | 概要 | リクエスト（パス/クエリ/ボディ） | レスポンス（主なデータ） |
| --- | --- | --- | --- |
| `GET /admin/companies/{company_id}/accounts` | この会社のアカウント一覧を取得（SC-92） | パス: `company_id`／クエリ: `q`（氏名/login_id/email）・`status`（`active\|disabled`）・`group_id`・`page`/`per_page`（オフセット）。**DataTable 契約（§1.8.1）**: ソート可能キー＝`display_name`/`login_id`/`email`/`system_role`/`status`/`last_login_at`/`created_at`／フィルタ可能＝`status`・`system_role`（enum・多値可）・`group_id`／`?format=csv`＝可（監査対象）／`?pin_ids=`＝可 | `data`=アカウントの配列。各行に氏名/`login_id`/`email`/**`email_verified`（bool・ADR-0009）**/`system_role`/`status`＋**`memberships[]`＝`{group_id, role, name}`**（有効所属＝`removed_at IS NULL`・**name＝所属クエストグループ列の表示名**・所属無しは `[]`）。`page_info.total` |
