...
| `GET /info-items/{id}` | 情報詳細（全属性＋カテゴリ＋関連リンク＋続報スレッド） | パス: `info_id` | 全属性（§5.33・`body_html`〔サニタイズ済リッチ〕/`summary`）＋`categories[]`（#8）＋`links[]`（`id`/`target_type`/`target_id`/`target_title`/`kind`/`origin`/`score`/`rejected`）＋`tokens_top[]`（ワードクラウド上位）＋`attachments[]`（参考資料・§5.33＝`id`/`original_name`/`size_bytes`/`mime_type`/`uploaded_by`/`uploaded_at`/`url`〔短TTL 署名〕）＋`thread`（`parent`〔続報元・あれば〕/`follow_ups[]`〔続報・時系列・§12-1〕）＋`can`（`edit_content`〔作成者＝内容編集〕/`curate`〔curator＝属性/triage/status/archive〕/`add_link`〔全員＝関連リンク〕/`follow_up`〔続報登録〕の可否＝サーバー算出）＋`content_revisions[]`〔内容の編集履歴・版/編集者/日時・Phase C〕 |
