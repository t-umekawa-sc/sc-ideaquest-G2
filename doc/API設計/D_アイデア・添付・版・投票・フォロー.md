...
| --- | --- | --- | --- |
| `GET /quests/{quest_id}/ideas` | クエスト内アイデア一覧＋自分の下書きを取得（SC-12 アイデアタブ） | パス: `quest_id`／クエリ: `q?`（件名/本文/価値 部分一致は検索＝J に委譲・ここは件名の簡易絞り）・`status?`（`published`〔既定〕`\|draft`〔自分の下書きのみ〕）・`sort`（`-created_at`〔新着〕/`-vote_count`/`-updated_at`）・`limit`/`cursor`（§1.8）。**DataTable 契約（§1.8.1）**: 上記 `sort` キーは**複数指定可**（カンマ区切り・カーソルがソートタプルを内包）／フィルタ可能＝`status`（enum）／横断 `q` の全文検索は §1.11 の PGroonga（`GET /quests/{id}/search`）に委譲＝ここは件名の簡易絞り／ページ方式＝**カーソル型**（「もっと見る」・番号ページャは使わない）／`?format=csv`・`?pin_ids=`＝SC-12 適用（ピンは list/card 共通・§4.5⑨） | `data`=アイデア行の配列（`id`/`title`/`status`/`author`〔アバター/Lv/氏名〕/`vote_summary`〔`approve`/`oppose` 数・匿名化考慮・下書きは 0〕/`comment_count`/`is_selected`/`current_revision`/`updated_at`/`my_vote`〔自分の投票・匿名でも本人には返す〕/`following`/`my_state`〔`draft`〔本人の下書き〕/未投票/投票済 等〕）。`page_info.{next_cursor,has_next}` |
| `GET /ideas/{idea_id}` | アイデア詳細を取得（SC-22） | パス: `idea_id` | アイデア本体（`title`/`value`/`body`/`stakeholders[]`/`time_limit`/`note`/`status`/`is_selected`/`current_revision`/`author`/`created_at`/`updated_at`）＋`quest`（`id`/`title`/`status`/`categories[]`/`deadline`＝SC-22 の「クエストへ戻る」導線・カテゴリーバッジ・凍結〔`completed`〕判定用）＋`attachments[]`（D.3 メタ）＋`vote`（`summary`＋`my_vote`＋`voters?`〔記名時のみ・D.5〕＋`stale`〔投票後更新フラグ・D.5〕）＋`following`（bool）＋`chat_preview`（直近数件＝ドメイン E・**現状未実装/将来対応**＝現行 DTO は内包しない・E.1【現状未実装】参照）＋`my_permissions`（UX 出し分け用） |
| `GET /ideas/{idea_id}/related-info` | このアイデアに関連づいた情報の一覧（SC-22 概要直下パネル・FR-41・N.1 委譲） | パス: `idea_id`／クエリ: `limit?`（既定＝上位 N） | 門番＝アイデア可視性（当該クエストのパーティー／`draft` は本人のみ・範囲外 404＝`GET /ideas/{id}` と同一）。`info_links`（`target_type='ideas'` ∧ `target_id=idea_id` ∧ `rejected_at IS NULL`）を **`score` 降順**（archived 除外）。`data[]`=`{link_id, info_id, title, kind, origin, score, source_url?, impact_class?, summary?, linked_by?〔manual のみ＝`info_links.created_by_id` を氏名/アバターに解決〕, disposition〔pending/adopted/declined〕, disposition_note?, disposed_by?, disposed_at?, can_dispose〔採否できるか＝作成者/所属クエスト owner・quest_admin〕}`（**不採用〔declined〕も返す**）。カードクリックで情報詳細（SC-52）。**反証（`refuting`）は⚠バッジ**。読取専用 |
| `PATCH /ideas/{idea_id}/related-info/{link_id}` | このアイデアに貼られたリンクの採否（disposition・FR-41 Phase2） | パス: `idea_id`/`link_id`／ボディ: `disposition`（`pending\|adopted\|declined`）・`note?` | 門番＝**アイデア作成者 or 所属クエスト owner/quest_admin**（可視は上と同じ・書込は管理権限者）。`disposition`/`disposition_note`/`disposed_by_id`/`disposed_at` を更新。`pending` 以外でリンクをロック（棄却/棄却解除/種別変更を 409・N.3-採否）／`pending` で解除。リンクが当該アイデアのものでなければ 404。更新後の related-info 行を返す |
...
- **ダッシュボード向けの横断 read（ドメイン I 連携・別 EP は新設しない）**: SC-01 が要する「全クエスト横断の自分の**下書きアイデア**・参加クエストの**未投票アイデア**（`my_vote IS NULL`）・**フォロー中アイデア一覧**（`follows`×`ideas`）」は、本ドメインの repository に read として持ち、**I が `GET /dashboard` 内で合成**する（`I_ダッシュボード集約.md` I.3）。門番（パーティー所属）・匿名化（`vote_summary`）は本ドメインの規則をそのまま適用＝I は再実装しない。単体画面を持たないため個別 REST は公開しない（必要になった時点で公開）。
...
- **ページング（§1.8）**: 本ドメインの一覧（アイデア一覧・版タイムライン等）は**カーソル方式（`limit`/`cursor`）を既定**とする（ユーザー向けフィード＝新着が増えても安定・§1.8 のオフセット `page`/`per_page` は管理系向けで本ドメインでは用いない）。
...
- **編集と版**: 公開済みアイデアの `PATCH` は**内容が変わった保存ごとに** `idea_revisions` に 1 版追加＋`ideas.current_revision++`＋投票者/フォロワーへ `idea_updated` 通知（D.4）。**下書き中の編集は版を作らない**（公開前は履歴不要・保存で上書き）。**無変更保存（差分ゼロ）はサーバーでも版/通知を作らない**＝`PATCH` 適用の前後で追跡対象フィールド（`title`/`value`/`body`/`time_limit`/`note`/`stakeholders`＋添付名一覧）のスナップショットが完全一致なら `_record_revision` をスキップ（200 は返す）。理由＝frontend の空更新抑制（`IdeaForm.persist`）と対称にサーバーを権威（コーディング規約 §1）とし、無意味な版・通知スパムを防ぐ。※「軽微だが実変更あり」の抑制（誤字修正の `notify:false` 等）は別問題＝D.8 未確定（ここで抑制するのは**差分ゼロ**のみ）。
...
- **版の生成（確定方針）**: **公開済みアイデアの内容が変わった `PATCH`（保存）ごとに 1 版**を `idea_revisions` に追加（`revision`＝`ideas.current_revision` を +1 した新値・`UNIQUE(idea_id, revision)`・§5.14）。`changes` に**その版時点の対象フィールド全値のスナップショット**（`title`/`value`/`body`/`time_limit`/`note`/`stakeholders`）を保存（§8-⑤ 決定）。差分は保存せず、`diff` エンドポイントで**表示時にサーバーが前版と比較して算出**（後から差分アルゴリズムを変えられる・監査に強い）。**差分ゼロの保存は版を作らない**（上記「編集と版」のサーバー無変更ガード）。
- **通知発火（FR-34）**: 版追加ごとに**投票者＋フォロワー**へ `idea_updated` 通知を発火（ドメイン H・`notifications.type=idea_updated`・`ref_idea_id`＋`ref_idea_revision_id` を持つ）。**版追加（`idea_revisions` 追加＋`ideas.current_revision++`）は本体 UoW**／**`idea_updated` の通知生成（H の `notify()`）と Redis publish（`notifications:{user_id}`）は本体コミット後の post-commit で実施**（best-effort＋冪等＝at-most-once・コーディング規約 §3.5-(3)・H.1）。**MVP は誤字修正も 1 版・通知あり**（軽微修正の抑制は D.8）。
...
- **初版 `revision=1` を公開処理時に記録（決定 2026-08-06）**: 公開処理（publish／作成公開）の同一 UoW で、**公開時点の内容スナップショットを `idea_revisions` に `revision=1` として 1 件記録**する（`ideas.current_revision` 既定 1・§5.10）。以後の公開中 `PATCH` が 2,3,… と増える。**なぜ記録するか**＝差分の起点（`diff` の `from` 既定）と `votes.voted_revision` 判定の基準を `revision=1` で常に存在させ、初版との差分・投票陳腐化を一貫して算出できるようにするため（初版が無いと最初の編集＝`revision=2` の差分起点が欠落する）。※通知は発火しない（公開自体の通知は D の公開処理・版通知は 2 版目以降の編集から）。
...
- **XP+5 は各アイデア初回のみ**（確定・§7/§8-⑥）: 当該ユーザー×当該アイデアで**過去に投票 XP を記帳していなければ**のみ `activities`（`reason=vote`,`ref_type=ideas`,`ref_id=idea_id`）を 1 件作成＝XP+5。**賛成/反対の切替・取消・再投票では追加付与しない**（冪等判定＝`activities` の存在／取消後の再投票でも再付与しない）。**日次上限＝XP 付与を伴う「初回投票が成立した回数」が 5 回/日**（同一アイデアへの賛成⇄反対の切替・取消・再投票は XP 付与を伴わないため上限の対象外／超過分は投票自体は成立するが XP 付与を抑止・サーバー判定・§8-⑥・データモデル §7）。
...
- **認可（2）**: 全エンドポイントでサーバー強制。**IDOR 対策**＝`idea_id`/`attachment_id` を書き換えても、パーティー所属・クエスト内6権限・投稿者本人性を都度照合し範囲外は **404/403**。可視範囲＝パーティー内のみ（会社/グループ全体へ漏らさない）。下書きは投稿者本人のみ。**クロステナント遮断**＝`company_id` はセッション由来（§1.5）。
