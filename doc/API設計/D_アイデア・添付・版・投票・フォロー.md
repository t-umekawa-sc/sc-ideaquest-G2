...
| `GET /ideas/{idea_id}/related-info` | このアイデアに関連づいた情報の一覧（SC-22 評価結果下パネル・FR-41・N.1 委譲） | パス: `idea_id`／クエリ: `limit?`（既定＝上位 N） | 門番＝アイデア可視性（当該クエストのパーティー／`draft` は本人のみ・範囲外 404＝`GET /ideas/{id}` と同一）。`info_links`（`target_type='ideas'` ∧ `target_id=idea_id` ∧ `rejected_at IS NULL`）を **`score` 降順**（archived 除外）。`data[]`=`{link_id, info_id, title, kind, origin, score, source_url?, impact_class?, summary?, linked_by?〔manual のみ＝`info_links.created_by_id` を氏名/アバターに解決＝**手動で関連付けた人**・auto は null〕}`。カードクリックで情報詳細（SC-52）。**反証（`refuting`）は⚠バッジ**。**Phase 1＝表示のみ**（リンクの追加/種別変更/棄却は情報側 N.3＝会社内全員／採否・処理は成果物側の別スコープ＝Phase 2）。読取専用 |
