...
- **クエスト完了（`quest_status=completed`）で書き込み凍結**（C.5）: アイデア作成/編集/削除・投票（登録/切替/取消）・添付の追加/削除は**すべて 409 `conflict`（`invalid_state`）**でサーバー拒否＝読み取り専用（完了済みクエストの共有状態を変更させない）。**フォローだけは例外だが「解除のみ」可**＝`DELETE /ideas/{id}/follow`（残存購読の後片付け・状態を汚さない）は許可し、**新規フォロー `POST /ideas/{id}/follow` は 409 で拒否**する（凍結後は通知対象イベント〔コメント/評価/選定/更新〕が一切発生せず、購読しても無意味なため）。詳細は D.6 注記。
