...
- 最新コミット＝**`ef370f8`**（ダッシュの最近の通知を realtime 追随）。本セッションのコミット＝`72c26ba`/`41b9fa9`/`d957bc6`（#12・**push 済み**）＋`a81ea3e`（ダッシュ週間ランキングのアバター imageUrl）＋`8986f86`（表彰台累積＋自分の行の白潰れ）＋`2c4c89d`（handoff）＋`306746c`（#13 演出追加＋GF-AC-130-133 受入）＋`ec5f37d`（handoff）＋`ef370f8`（ダッシュ最近の通知 realtime 追随）。**`a81ea3e`〜`ef370f8` の 6 コミットは未 push**（`origin/main` より ahead 6・都度確認）。
...
## 7. 次にやること（優先順・具体的に）
1. **未 push 4 コミットの push（都度確認）**＝`a81ea3e`〜`306746c`（#12 は push 済み・#13 とアバター修正が未 push）。
2. **受入状況**＝#11 ✅／#12 ✅／**#13 ✅（表彰台減速＋自分アバターのジャンプ追加）**／**#14 仮OK（3D アバター未実装のため暫定）**／**#15 ✅（GF-AC-150/151 目視・152 は Claude e2e G-TC-170）**。加えて**ダッシュの最近の通知を realtime 追随に変更**（`ef370f8`・SC-01 §4.8 設計メモ）。**本セッションのコミットは全て push 済み（`151a2bf` で origin と同期）**。
3. **積み残しの GF-AC バックログ**＝#16 クエスト選定の祝福(160..)／#17 以降／…。ID 順に、機能は先に私がテスト＋実起動確認してから見た目をユーザーへ。
4. **backend pytest の再確認**（任意）＝§8 手順で 513 相当が緑か。
5. **e2e login ヘルパの「ようこそ」待ちは全 spec で古い**（greeting.ts は時間帯で おはよう/こんにちは/こんばんは＝「ようこそ」無し）。sc-41/sc-02 は URL＋`.app-header` 判定へ修正済み。他 spec（sc-01 等）にも残存の可能性＝赤ければ同修正。
