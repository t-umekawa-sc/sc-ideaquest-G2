...
- **次に確認するとよい TBD（推奨順）**:
  1. **XP・コインの付与ルールとレベル設計**（どの行動でいくつ付与するか＝アプリの根幹。投票/評価/チャット/ログインも対象にするか）。
  2. **アイデア各項目の必須/任意**（タイムリミット・利害関係者・価値）。すぐ決められて画面設計に効く。
  3. **チャットの参加範囲**（パーティー全員か限定か）・添付/メンション/通知の有無。
  4. **5観点の重み付けの有無**（現状は均等）。
  5. 装備アイテムの種類・入手方法、3Dアバターのモデル形式（glTF/VRM）・着せ替え方式。
