...
> - ✅ **M5 修正済（サーバー＋spend系フロント）**＝横断ミドルウェア `app/core/idempotency.py`（会社×アカウント×キーで Redis スコープ・24h・`SET NX` in_flight→done・replay/in_progress/key_reuse・2xx/決定的4xx のみ保存・5xx 解放・commit 後保存）。`main.py` に `add_request_id` の内側で登録＝**POST＋`Idempotency-Key` ヘッダ有りのみ作動**（未付与 POST は素通し＝既存 479 テスト不変）。フロント `idempotencyHeader()`＝**購入/魔法解放は配線済み**。SEC-TC-041-044（red→green）。**残＝フロント配線をアイデア投稿/クエスト作成/評価確定へ順次拡大**（サーバーは既に全 POST 対応・クライアントが header を送れば作動）。
...
> - 🟡 **F1 backend 完了（フロント残）**＝FR-36 アクティビティフィード ②`GET /quests/{id}/activities`（SC-12・門番＝パーティー所属）・③`GET /me/feed`（SC-01・参加クエスト横断・各行 quest 付き）を実装＝**公開種別のみ**（idea_post/selection/achievement_reward/levelup_sp）を `gami_repo.PUBLIC_FEED_REASONS`＋クエリで強制。カーソル（created_at,id）。G-TC-109/110（api・green）。**残＝フロント**＝SC-12 §4.1c クエスト内フィード・SC-01 §4.8b チームフィードの demo パネルを実接続（OpenAPI 型再生成＋各行の人間可読レンダリング＝reason/ref/actor から。ref リンクは D/E 依存）。
