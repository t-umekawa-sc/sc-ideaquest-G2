...
>
> **次回の開始点＝(1) ユーザーが `doc/API設計/L_リアルタイム配信.md` を確認中 → (2) その後、私が API設計 A〜L を横断再レビューして矛盾/drift を洗い出す（読み取りのみ・指摘一覧を出すだけで修正しない）→ (3) 以降の進め方（実装スキャフォールド）を検討**（§7 参照）。
...
### (1) ユーザーの `L_リアルタイム配信.md` 内容確認を待つ
- **ユーザーが `doc/API設計/L_リアルタイム配信.md`（L.0〜L.5）を確認中**。指摘が出たら、その論点を整理→決定案提示→承認→反映（2段コミット）。**確認・承認が取れるまでドキュメントは変更しない**。

### (2) ★最優先タスク＝API設計 A〜L の横断再レビュー（矛盾/drift の洗い出し）
- **ユーザー依頼の合意済みタスク**。`doc/API設計/A_…`〜`L_…` と `README.md`（§1 全体規約）＋`doc/データモデル.md` を通読し、**ドメイン間の矛盾・drift を洗い出す**。
- **やり方＝読み取りのみ。指摘を「矛盾箇所の一覧＋重大度」として提示するに留め、修正はしない**（ユーザー承認後にまとめて反映）。範囲が広いので観点別に並行調査してよい（Explore/Agent 活用可）。
- **突き合わせる主な観点（具体）**:
