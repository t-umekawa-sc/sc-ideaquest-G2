...
2. **3D アバター（FR-08）＝仕様有るが未実装・テストパターンも無し**（監査で判明）。着手前に (a) `doc/要件定義/README.md` の FR-08 と該当画面設計 `doc/画面設計/screens/SC-xx_*.md` を開き、(b) `doc/テスト/` に TC 行を先に起こし、(c) **ユーザーへスコープ確認**（3D の描画方式・データ源が未確定のため）。
