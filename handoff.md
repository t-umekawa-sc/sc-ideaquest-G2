...
- **複製はパーティーも引き継ぐ**（2026-09-13 変更）＝件名/カラー/カテゴリ/参加グループ/期限/目的に加え、作成者以外の参加メンバーを権限・所属グループ込みで引き継ぐ（テンプレート的に使えるように）。作成者・id・ステータス・アイコンは引き継がない。旧「パーティーは引き継がない」方針を上書き。

## 7. 次にやること（優先順・具体）
1. **実装計画の残ドメイン**＝`doc/実装計画.md` の順（アカウント→クエスト→アイデア→評価→その他）で未接続画面が残っていれば backend 接続を1画面単位で進める（現況は `impl/README.md` が正）。着手前に `impl/README.md` で現況把握。
2. UI 微調整の余地＝カード件名フォント/アイコンサイズ、リスト列幅の数値、`GROUP_CHIP_MAX`（現状2）などはユーザーフィードバック次第で調整可能。

### 消化済み（本セッション後半）
- **複製（duplicate）の引き継ぎ強化**＝参加グループ（`quest_group_ids`）に加え**パーティー（members＝作成者以外・権限/所属グループ込み）**も引き継ぐよう実装（`QuestListView` 複製→ dup プリフィルに `quest_group_ids`/`members`・`QuestForm` が作成モードで初期化）。決定＝§6。
