...
- 装備アイテムの種類・価格・入手方法（購入のみか報酬でも入手か）。
- 添付ファイルの上限サイズ・形式・保管先。
...
- **次に確認するとよい TBD（推奨順）**:
  1. **装備アイテムの設計**（種類＝頭/体/アクセサリ等・価格・入手方法）。コイン付与が決まったので価格設計が可能に。ショップ/アバターの根幹。
  2. **5観点の重み付けの有無**（現状は均等）。評価集計・コイン付与額に直結。
  3. **チャットの添付/メンション/通知**の有無（参加範囲＝パーティー全員は決定済み）。
  4. **XP日次上限の具体値**・コイン付与の確定タイミング（評価締切一括 or 都度再計算）・レベル上限の要否。
  5. 3Dアバターのモデル形式（glTF/VRM）・着せ替え方式、添付ファイルの上限/保管先。
