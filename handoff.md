...
- 最新コミット（本文＝直近の内容変更）: **デイリーログインボーナスの付与契機を「新しいJST日の最初の認証済みリクエスト」に一般化**（`61c4a4a`）。その前が 規約 §2.3「DRY」追記（`966943b`）／API設計 ドメインI 詳細確定（`b7644d5`）。**本 handoff 追記コミットはこの直後**（2 段方式）。
...
- 直近の主なコミット（新しい順・ハッシュは `git log --oneline` で確認）:
  - `61c4a4a` API設計: デイリーログインボーナスの付与契機を「新しいJST日の最初の認証済みリクエスト」に一般化
  - `966943b` 規約: コーディング規約 §2.3「DRY（単一の情報源）と重複許容の線引き」追記
...
今セッション＝**(1) ドメインH のレビュー反映（B-1〜C-6）**、**(2) ⑦ドメインI（ダッシュボード集約／SC-01）の詳細確定**、**(3) コーディング規約 §2.3「DRY」追記**。変更は **ドキュメントのみ**（コードは無い）。各段階で「本体→handoff にハッシュ追記」の2段コミット・プッシュ済み。

### 3.-1 デイリーログインボーナスの付与契機を一般化（コミット `61c4a4a`）
- **問題**＝ログイン XP（`reason=login`）を「A のログイン成功」だけに紐づけると、持続セッション（アイドル延長＋絶対有効期限）で日を跨ぐ利用者が日次XPと連続ログイン実績を取り逃す。
- **決定**＝付与契機を **「新しい暦日（JST）の最初の認証済みリクエスト」**（ログイン成功を含む）に一般化＝セッション寿命から切り離す。**機構の3分割**＝判定=純粋関数（G domain `daily_login_bonus_due`）／付与=冪等な台帳（`activities(reason=login)` を**ユーザー×JST日で1回**・存在チェック）／契機=セッション解決依存性（A の imperative shell・当日初回のみ G へ委譲）。業務ロジックをミドルウェアに直書きしない（§3.1）・付与ドメインは G（§3.5-(2)）。**日境界は JST 統一**（ランキング週起点と同一）。**スキーマ変更なし**。
- **反映**＝A.1／G.6（＋注記）／データモデル §7・activities 注記／SC-00 §10／I.1・委譲。

### 3.0 コーディング規約 §2.3「DRY」追記（コミット `966943b`）
- DRY 相当は §3.1/§3.4/§3.5-(2)/§2/§2.1/§4.1 に散在していたが名前付き原則として未集約だったため、§2（横断規約）に **§2.3「DRY（単一の情報源）と重複許容の線引き」** を新設。DRY＝「知識（業務ルール・定義）の正は1箇所」と限定し既定の置き場所へ相互参照（再掲せず drift 回避）＋**適用の限界**（たまたま似たコードを無理に共通化しない／早すぎる抽象化をしない〔3回目で検討〕／間違った抽象より重複が安い）を併記＝§3.5 の反・過剰設計思想と整合。
...
- ログインボーナス（再検討不要）: 付与契機＝「新しいJST日の最初の認証済みリクエスト」（認証イベントに限らない）／判定=純粋・付与=冪等台帳（ユーザー×JST日1回）・契機=セッション解決依存性／日境界 JST／スキーマ変更なし。
