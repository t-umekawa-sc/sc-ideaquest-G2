...
- **残（Phase D 最後）**＝`kind=refuting`→対象の作成者+評価者へ通知＋要再評価（per-link・§N.6・通知 H ドメイン＋成果物側の要再評価フラグ＝他ドメイン連携で重い・設計判断要）。**「属性を編集」行メニューは fixtures な /edit フォームへ飛ぶ＝詳細インライン編集に寄せる整理が別途必要（小）**。
