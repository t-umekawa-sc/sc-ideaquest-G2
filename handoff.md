...
2. **受入状況**＝#11 ✅／#12 ✅（GF-AC-120/121/122）／**#13 ✅（GF-AC-130/131/132/133・表彰台減速＋自分アバターのジャンプ追加）**／**#14 仮OK（3D アバター未実装のため暫定・GF-AC-140/141/142）**。**#15 通知の新着 pop は未確認**＝発火はリアルタイム新着（未読増）＝**別ユーザー/別セッションであなた宛の通知を発生**させて確認（GF-AC-152 reduce は Claude が e2e で担保可）。
3. **積み残しの GF-AC バックログ**＝#15 通知 pop(150..152)／#16 クエスト選定の祝福(160..)／…。ID 順に、機能は先に私がテスト＋実起動確認してから見た目をユーザーへ。
